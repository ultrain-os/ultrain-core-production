...
def monitor_exchange():
    ...
    results = clultrain('get actions exchange {} 0 -j'.format(action_num))
...
def transfer(to, quantity):
    if quantity[:-4] != ' ULTRAIN':
        quantity += ' ULTRAIN'
    results = clultrain('transfer exchange {} "{}" {} -j'.format(to, quantity, KEY_TO_INTERNAL_ACCOUNT))
...
def is_valid_deposit(action):
    ...
    if (account == 'ultrainio.token' and
