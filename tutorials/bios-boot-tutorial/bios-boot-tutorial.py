...
fastUnstakeSystem = './fast.refund/ultrainio.system/ultrainio.system.wasm'
...
def importKeys():
    run(args.clultrain + 'wallet import ' + args.private_key)
    keys = {}
    for a in accounts:
        key = a['pvt']
        if not key in keys:
            if len(keys) >= args.max_user_keys:
                break
            keys[key] = True
            run(args.clultrain + 'wallet import ' + key)
...
def createStakedAccounts(b, e):
    ...
    for i in range(b, e):
        ...
        retry(args.clultrain + 'system newaccount --transfer ultrainio %s %s --stake-net "%s" --stake-cpu "%s" --buy-ram "%s"   ' % 
            (a['name'], a['pub'], intToCurrency(stakeNet), intToCurrency(stakeCpu), intToCurrency(ramFunds)))
        if unstaked:
            retry(args.clultrain + 'transfer ultrainio %s "%s"' % (a['name'], intToCurrency(unstaked)))

def regProducers(b, e):
    for i in range(b, e):
        a = accounts[i]
        retry(args.clultrain + 'system regproducer ' + a['name'] + ' ' + a['pub'] + ' https://' + a['name'] + '.com' + '/' + a['pub'])
...
def vote(b, e):
    for i in range(b, e):
        ...
        retry(args.clultrain + 'system voteproducer prods ' + voter + ' ' + prods)

def claimRewards():
    table = getJsonOutput(args.clultrain + 'get table ultrainio ultrainio producers -l 100')
    times = []
    for row in table['rows']:
        if row['unpaid_blocks'] and not row['last_claim_time']:
            times.append(getJsonOutput(args.clultrain + 'system claimrewards -j ' + row['owner'])['processed']['elapsed'])
...
def proxyVotes(b, e):
    ...
    retry(args.clultrain + 'system regproxy ' + proxy)
    sleep(1.0)
    for i in range(b, e):
        voter = accounts[i]['name']
        retry(args.clultrain + 'system voteproducer proxy ' + voter + ' ' + proxy)
...
def randomTransfer(b, e):
    for j in range(20):
        ...
        run(args.clultrain + 'transfer -f ' + src + ' ' + dest + ' "0.0001 ' + args.symbol + '"' + ' || true')
...
def msigProposeReplaceSystem(proposer, proposalName):
    ...
    trxPermissions = [{'actor': 'ultrainio', 'permission': 'active'}]
    with open(fastUnstakeSystem, mode='rb') as f:
        setcode = {'account': 'ultrainio', 'vmtype': 0, 'vmversion': 0, 'code': f.read().hex()}
    run(args.clultrain + 'multisig propose ' + proposalName + jsonArg(requestedPermissions) + 
        jsonArg(trxPermissions) + 'ultrainio setcode' + jsonArg(setcode) + ' -p ' + proposer)

def msigApproveReplaceSystem(proposer, proposalName):
    for i in range(firstProducer, firstProducer + numProducers):
        run(args.clultrain + 'multisig approve ' + proposer + ' ' + proposalName +
            jsonArg({'actor': accounts[i]['name'], 'permission': 'active'}) +
            '-p ' + accounts[i]['name'])

def msigExecReplaceSystem(proposer, proposalName):
    retry(args.clultrain + 'multisig exec ' + proposer + ' ' + proposalName + ' -p ' + proposer)

def msigReplaceSystem():
    run(args.clultrain + 'push action ultrainio buyrambytes' + jsonArg(['ultrainio', accounts[0]['name'], 200000]) + '-p ultrainio')
    sleep(1)
...
def produceNewAccounts():
    with open('newusers', 'w') as f:
        for i in range(120_000, 200_000):
            x = getOutput(args.clultrain + 'create key')
...
def stepKillAll():
    run('killall kultraind nodultrain || true')
    sleep(1.5)
def stepStartWallet():
    startWallet()
    importKeys()
def stepStartBoot():
    startNode(0, {'name': 'ultrainio', 'pvt': args.private_key, 'pub': args.public_key})
    sleep(1.5)
def stepInstallSystemContracts():
    run(args.clultrain + 'set contract ultrainio.token ' + args.contracts_dir + 'ultrainio.token/')
    run(args.clultrain + 'set contract ultrainio.msig ' + args.contracts_dir + 'ultrainio.msig/')
def stepCreateTokens():
    run(args.clultrain + 'push action ultrainio.token create \'["ultrainio", "10000000000.0000 %s"]\' -p ultrainio.token' % (args.symbol))
    totalAllocation = allocateFunds(0, len(accounts))
    run(args.clultrain + 'push action ultrainio.token issue \'["ultrainio", "%s", "memo"]\' -p ultrainio' % intToCurrency(totalAllocation))
    sleep(1)
def stepSetSystemContract():
    retry(args.clultrain + 'set contract ultrainio ' + args.contracts_dir + 'ultrainio.system/')
    sleep(1)
    run(args.clultrain + 'push action ultrainio setpriv' + jsonArg(['ultrainio.msig', 1]) + '-p ultrainio@active')
...
def stepResign():
    resign('ultrainio', 'ultrainio.prods')
    for a in systemAccounts:
        resign(a, 'ultrainio')
def stepTransfer():
    while True:
...
def stepLog():
    run('tail -n 60 ' + args.nodes_dir + '00-ultrainio/stderr')
...
commands = [
    ...
    ('s', 'sys',            createSystemAccounts,       True,    "Create system accounts (ultrainio.*)"),
    ...
    ('q', 'resign',         stepResign,                 True,    "Resign ultrainio"),
    ('m', 'msg-replace',    msigReplaceSystem,          False,   "Replace system contract using msig"),
    ('X', 'xfer',           stepTransfer,               False,   "Random transfer tokens (infinite loop)"),
    ('l', 'log',            stepLog,                    True,    "Show tail of node's log"),
]

parser.add_argument('--public-key', metavar='', help="UTRIO Public Key", default='ULTRAIN8Znrtgwt8TfpmbVpTKvA2oB8Nqey625CLN8bCN3TEbgx86Dsvr', dest="public_key")
parser.add_argument('--private-Key', metavar='', help="UTRIO Private Key", default='5K463ynhZoCDDa4RDcr63cUwWLTnKqmdcoTKTHBjqoKfv4u5V7p', dest="private_key")
parser.add_argument('--clultrain', metavar='', help="Clultrain command", default='../../build/programs/clultrain/clultrain --wallet-url http://localhost:6666 ')
parser.add_argument('--nodultrain', metavar='', help="Path to nodultrain binary", default='../../build/programs/nodultrain/nodultrain')
parser.add_argument('--kultraind', metavar='', help="Path to kultraind binary", default='../../build/programs/kultraind/kultraind')
parser.add_argument('--contracts-dir', metavar='', help="Path to contracts directory", default='../../build/contracts/')
parser.add_argument('--nodes-dir', metavar='', help="Path to nodes directory", default='./nodes/')
parser.add_argument('--genesis', metavar='', help="Path to genesis.json", default="./genesis.json")
parser.add_argument('--wallet-dir', metavar='', help="Path to wallet directory", default='./wallet/')
parser.add_argument('--log-path', metavar='', help="Path to log file", default='./output.log')
parser.add_argument('--symbol', metavar='', help="The ultrainio.system symbol", default='SYS')
