...
reduced. And to achieve true fairness and randomness of proposers selection, we devise a novel on-chain random number generator that
...
![alt text](../tools/pics/sidechain.png "Multichain")
...
a new shard-chain by the main-chain’s smart contract. The most common way of data sync on the blockchain is the block replay model –it syncs al
the history blocks and replay all the transactions on the shard-chain to build the latest world state for the miner node before it can
...
which are also provided "AS  IS” without warranty of any kind. Without limiting the generality of the foregoing, ULTRAIN.I
