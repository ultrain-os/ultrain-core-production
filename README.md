## ULTRAIN.IO - The Most Powerful Infrastructure for Decentralized Applications

[![Build Status](https://jenkins.ultrain.io/buildStatus/icon?job=ultrainio/master)](https://jenkins.ultrain.io/job/ultrainio/job/master/)

Welcome to the ULTRAIN.IO source code repository!  ULTRAIN.IO software enables developers to create and deploy high-performance, horizontally scalable, blockchain infrastructure upon which decentralized applications can be built.
...
The public testnet described in the [wiki](https://github.com/ULTRAINIO/ultrain/wiki/Testnet%3A%20Public) is running the `dawn-2.x` branch.  The `master` branch is no longer compatible with the public testnet.  Instructions are provided below for building a local testnet using the `master` branch.  This document will be updated later with instructions for running on the `dawn-3.x` public testnet.

### Supported Operating Systems
ULTRAIN.IO currently supports the following operating systems:  
...
2. [Documentation](https://ultrainio.github.io/ultrain/)
3. [Blog](https://steemit.com/@ultrainio)
4. [Community Telegram Group](https://t.me/ULTRAINProject)
5. [Developer Telegram Group](https://t.me/joinchat/EaEnSUPktgfoI-XPfMYtcQ)
6. [White Paper](https://github.com/ULTRAINIO/Documentation/blob/master/TechnicalWhitePaper.md)
7. [Roadmap](https://github.com/ULTRAINIO/Documentation/blob/master/Roadmap.md)
8. [Wiki](https://github.com/ULTRAINIO/ultrain/wiki)
...
6. [Running ULTRAIN in Docker](#docker)
...
To download all of the code, clone the ultrain repository and its submodules.

```bash
git clone https://github.com/ultrainio/ultrain --recursive
```
...
ULTRAIN comes with a number of programs:

* nodultrain - server-side blockchain node component
* clultrain - command line interface to interact with the blockchain
* kultraind - ULTRAIN wallet
* ultrainio-launcher - application for nodes network composing and deployment; [more on ultrainio-launcher](https://github.com/ULTRAINIO/ultrain/blob/master/testnet.md)

The build places content in the `ultrain/build` folder.  The executables can be found in subfolders within the `ultrain/build/programs` folder.

<a name="autobuild"></a>
#### Automated build script

There is an automated build script that can install all dependencies and build ULTRAIN.  We are working on supporting other Linux/Unix distributions in future releases.
...
To manually build, use the following steps to create a `build` folder within your `ultrain` folder and then perform the build.  The steps below assume the `ultrain` repository was cloned into your home (i.e., `~`) folder.

```bash
cd ~
mkdir -p ~/ultrain/build && cd ~/ultrain/build
...
After successfully building the project, the `nodultrain` binary should be present in the `build/programs/nodultrain` folder.  `nodultrain` can be run directly from the `build` folder using `programs/nodultrain/nodultrain`.

`nodultrain` uses a custom configuration folder.  The location of this folder is determined by your system.

- Mac OS: `~/Library/Application Support/ultrainio/nodultrain/config`
- Linux: `~/.local/share/ultrainio/nodultrain/config`

The build seeds this folder with a default `genesis.json` file.  Alternatively, a configuration folder can be specified using the `--config-dir` command line argument to `nodultrain`.  If you use this option, you will need to manually copy a genesis.json file to your config folder.

`nodultrain` will need a properly configured `config.ini` file in order to do meaningful work.  On startup, `nodultrain` looks in the config folder for `config.ini`.  If one is not found, a default `config.ini` file is created.  If you do not
already have a `config.ini` file ready to use, run `nodultrain` and then close it immediately with <kbd>Ctrl-C</kbd>.  A default configuration (`config.ini`) will have been created in the config folder.  Edit the `config.ini` file, adding/updating the following settings to the defaults already in place:

```
# Load the testnet genesis state, which creates some initial block producers with the default key
genesis-json = /path/to/ultrain/source/genesis.json
# Enable production on a stale chain, since a single-node test chain is pretty much always stale
enable-stale-production = true
# Enable block production with the testnet producers
producer-name = ultrainio
# Load the block producer plugin, so you can produce blocks
plugin = ultrainio::producer_plugin
# Wallet plugin
...
Now it should be possible to run `nodultrain` and see it begin producing blocks.

When running `nodultrain` you should get log messages similar to below. It means the blocks are successfully produced.
...
`nodultrain` stores runtime data (e.g., shared memory and log content) in a custom data folder.  The location of this folder is determined by your system.

- Mac OS: `~/Library/Application Support/ultrainio/nodultrain/data`
- Linux: `~/.local/share/ultrainio/nodultrain/data`

A data folder can be specified using the `--data-dir` command line argument to `nodultrain`.

<a name="localtestnet"></a>
### Running a multi-node local testnet

To run a local testnet you can use the `ultrainio-launcher` application provided in the `~/ultrain/build/programs/ultrainio-launcher` folder.
...
cd ~/ultrain/build
cp ../genesis.json ./
./programs/ultrainio-launcher/ultrainio-launcher -p2 --skip-signature
```
...
```bash
spawning child, programs/nodultrain/nodultrain --skip-transaction-signatures --data-dir tn_data_0
spawning child, programs/nodultrain/nodultrain --skip-transaction-signatures --data-dir tn_data_1
```

To confirm the nodes are running, run the following `clultrain` commands:
```bash
~/ultrain/build/programs/clultrain
./clultrain -p 8888 get info
./clultrain -p 8889 get info
```

For each command, you should get a JSON response with blockchain information.

You can read more on ultrainio-launcher and its settings [here](https://github.com/ULTRAINIO/ultrain/blob/master/testnet.md)

<a name="publictestnet"></a>
### Running a local node connected to the public testnet

The `master` branch is no longer compatible with the `dawn-2.x` public testnet.  To run on the public testnet, please see [dawn-2.x/ultrain/README.md](https://github.com/ULTRAINIO/ultrain/blob/dawn-2.x/README.md)

## Next Steps
<a name="nextsteps"></a>

Further documentation is available in the [wiki](https://github.com/ULTRAINIO/ultrain/wiki). Wiki pages include detailed reference documentation for all programs and tools and the database schema and API. The wiki also includes a section describing smart contract development. A simple walkthrough of the "currency" contract follows.

<a name="smartcontracts"></a>
## Example "Currency" Contract Walkthrough

ULTRAIN comes with example contracts that can be uploaded and run for testing purposes. Next we demonstrate how to upload and interact with the sample contract "currency".
...
```bash
cd ~/ultrain/build/programs/nodultrain/
./nodultrain
```
...
To create a wallet, you need to have the wallet_plugin loaded into the nodultrain process which can be accomplished in one of two ways:
* Via a plugin entry in the config.ini file (i.e. `plugin = ultrainio::wallet_plugin`)
* Via a plugin command-line option when invoking nodultrain (i.e. `--plugin ultrainio::wallet_plugin`)

Use the wallet create command of the clultrain tool, to create a wallet.

```bash
cd ~/ultrain/build/programs/clultrain/
./clultrain wallet create # Outputs a password that you need to save to be able to lock/unlock the wallet
```
...
```bash
cd ~/ultrain/build/programs/clultrain/
./clultrain create key # owner-key
./clultrain create key # active-key
```
...
Public key: ULTRAINXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX
```

**Important:**
Save the values for future reference.

Run the `clultrain create account` command where `ultrainio` is the account authorizing the creation of the `currency` account and `public-owner-key` and `public-active-key` are the values generated above by the two invocations of the `clultrain create key` command

```bash
./clultrain create account ultrainio currency public-owner-key public-active-key
```
...
```bash
./clultrain get account currency
```
...
```json
{
  ...
  "ultrain_balance": "0.0000 UTR",
  "staked_balance": "0.0001 UTR",
  "unstaking_balance": "0.0000 UTR",
...
```bash
./clultrain wallet import XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX
```
...
```bash
./clultrain set contract currency ../../contracts/currency/currency.wast ../../contracts/currency/currency.abi
```
...
```bash
./clultrain push action currency issue '{"to":"currency","quantity":"1000.0000 CUR","memo":""}' --permission currency@active
```

Next verify the currency contract has the proper initial balance:

```bash
./clultrain get table currency currency account
{
  "rows": [{
     "currency": 1381319428,
     "balance": 10000000
...
The content of the message is `'{"from":"currency","to":"ultrainio","quantity":"20.0000 CUR","memo":"any string"}'`. In this case we are asking the currency contract to transfer funds from itself to someone else. This requires the permission of the currency contract.

```bash
./clultrain push action currency transfer '{"from":"currency","to":"ultrainio","quantity":"20.0000 CUR","memo":"my first transfer"}' --permission currency@active
```
...
```bash
./clultrain push action currency transfer '{"from":"${usera}","to":"${userb}","quantity":"20.0000 CUR","memo":""}' --permission ${usera}@active
```
...
```bash
./clultrain get table ultrainio currency account
{
  "rows": [{
      "currency": 1381319428,
      "balance": 200000
...
As expected, the receiving account **ultrainio** now has a balance of **20** tokens, and the sending account now has **20** less tokens than its initial supply.
...
For the `master` branch: https://ultrainio.github.io/ultrain/  
For the public testnet branch: http://htmlpreview.github.io/?https://github.com/ULTRAINIO/ultrain/blob/dawn-2.x/docs/index.html  

<a name="docker"></a>
## Running ULTRAIN in Docker

You can find up to date information about ULTRAIN Docker in the [Docker Readme](https://github.com/ULTRAINIO/ultrain/blob/master/Docker/README.md)
...
Your environment is set up. Now you can <a href="#runanode">build ULTRAIN and run a node</a>.
