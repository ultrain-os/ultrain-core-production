...
namespace ultrainiosystem {
   ...
   struct block_header {
      ...
      ULTRAINLIB_SERIALIZE(block_header, (previous)(timestamp)(transaction_mroot)(action_mroot)(block_mroot)
   ...
   class contract : public delegate_bandwidth<SystemAccount>, public native<SystemAccount> {
      public:
         ...
         ACTION( SystemAccount, nonce ) {
            ultrainio::string                   value;

            ULTRAINLIB_SERIALIZE( nonce, (value) )
         };
         ...
         ACTION(SystemAccount, onblock) {
            ...
            ULTRAINLIB_SERIALIZE(onblock, (header))
         };
         ...
         static void on(const onblock& ob) {
            ...
            const system_token_type to_ultrain_bucket = num_of_payments * parameters.payment_to_ultrain_bucket;
            parameters.last_bucket_fill_time = ob.header.timestamp;
            parameters.ultrain_bucket += to_ultrain_bucket;
            global_state_singleton::set(parameters);
         }
         ...
         ACTION(SystemAccount, claimrewards) {
            ...
            ULTRAINLIB_SERIALIZE(claimrewards, (owner))
         };

         static void on(const claimrewards& cr) {
            require_auth(cr.owner);
            ultrainio_assert(current_sender() == account_name(), "claimrewards can not be part of a deferred transaction");
            producers_table producers_tbl(SystemAccount, SystemAccount);
            auto prod = producers_tbl.find(cr.owner);
            ultrainio_assert(prod != producers_tbl.end(), "account name is not in producer list");
            ultrainio_assert(prod->active(), "producer is not active"); // QUESTION: Why do we want to prevent inactive producers from claiming their earned rewards?
            if( prod->last_rewards_claim > 0 ) {
               ultrainio_assert(now() >= prod->last_rewards_claim + seconds_per_day, "already claimed rewards within a day");
            }
            ...
            if (is_among_payed_producers && total_producer_votes > 0) {
               if( global_state_singleton::exists() ) {
                  ...
                  auto share_of_ultrain_bucket = system_token_type( static_cast<uint64_t>( (prod->total_votes * parameters.ultrain_bucket.quantity) / total_producer_votes ) ); // This will be improved in the future when total_votes becomes a double type.
                  rewards += share_of_ultrain_bucket;
                  parameters.ultrain_bucket -= share_of_ultrain_bucket;
                  global_state_singleton::set(parameters);
               }
            }

            ultrainio_assert( rewards > system_token_type(), "no rewards available to claim" );
         ...
         static void apply( account_name receiver, account_name code, action_name act ) {
            if ( !ultrainio::dispatch<currency, typename currency::transfer, typename currency::issue>( code, act ) ) {
               if( !ultrainio::dispatch<contract, typename delegate_bandwidth<SystemAccount>::delegatebw,
                                 ...
                                 nonce>( code, act) ) {
                  //TODO: Small hack until we refactor ultrainio.system like ultrainio.token
                  ...
                  if(code == undelegatebw::get_account() && act == undelegatebw::get_name() ){
                     contract().on( receiver, ultrainio::unpack_action_data<undelegatebw>() );
                  }
               }
            }
