...
namespace ultrainiosystem {
   ...
   class voting {
      public:
         ...
         using ultrainio_parameters = typename common<SystemAccount>::ultrainio_parameters;
         ...
         struct producer_info {
            ...
            ultrainio_parameters  prefs;
            ultrainio::bytes      packed_key; /// a packed public key object
            ...
            ULTRAINLIB_SERIALIZE( producer_info, (owner)(total_votes)(prefs)(packed_key)
                              (per_block_payments)(last_rewards_claim)
                              (time_became_active)(last_produced_block_time) )
         };

         typedef ultrainio::multi_index< N(producerinfo), producer_info,
         ...
         struct voter_info {
            ...
            ULTRAINLIB_SERIALIZE( voter_info, (owner)(proxy)(last_update)(is_proxy)(staked)(unstaking)(unstake_per_week)(proxied_votes)(producers)(deferred_trx_id)(last_unstake_time) )
         };
         ...
         ACTION( SystemAccount, regproducer ) {
            ...
            ULTRAINLIB_SERIALIZE( regproducer, (producer)(producer_key)(prefs) )
         };
         ...
         ACTION( SystemAccount, unregprod ) {
            ...
            ULTRAINLIB_SERIALIZE( unregprod, (producer) )
         };
         ...
         static void on( const unregprod& unreg ) {
            ...
            ultrainio_assert( prod != producers_tbl.end(), "producer not found" );
         ...
         static void increase_voting_power( account_name acnt, system_token_type amount ) {
            ...
            if ( voter->proxy ) {
               ...
               ultrainio_assert( proxy != voters_tbl.end(), "selected proxy not found" ); //data corruption
            ...
            if ( producers ) {
               ...
               for( auto p : *producers ) {
                  ...
                  ultrainio_assert( prod != producers_tbl.end(), "never existed producer" ); //data corruption
         ...
         static void decrease_voting_power( account_name acnt, system_token_type amount ) {
            ...
            voters_table voters_tbl( SystemAccount, SystemAccount );
            auto voter = voters_tbl.find( acnt );
            ultrainio_assert( voter != voters_tbl.end(), "stake not found" );

            if ( 0 < amount.quantity ) {
               ultrainio_assert( amount <= voter->staked, "cannot unstake more than total stake amount" );
         ...
         static void update_elected_producers(time cycle_time) {
            ...
            ultrainio::producer_schedule schedule;
            ...
            for ( auto it = idx.crbegin(); it != idx.crend() && n < 21 && 0 < it->total_votes; ++it ) {
               if ( it->active() ) {
                  ...
                  ultrainio_assert( sizeof(schedule.producers.back().block_signing_key) == it->packed_key.size(), "size mismatch" );
            ...
            // should use producer_schedule_type from libraries/chain/include/ultrainio/chain/producer_schedule.hpp
            ...
            parameters.payment_to_ultrain_bucket = payment_per_block(other_half_of_percentage);
            ...
            auto issue_quantity = parameters.blocks_per_cycle * (parameters.payment_per_block + parameters.payment_to_ultrain_bucket);
         ...
         ACTION( SystemAccount, voteproducer ) {
            ...
            ULTRAINLIB_SERIALIZE( voteproducer, (voter)(proxy)(producers) )
         };
          ...
          *  @pre voter must have previously staked some ULTRAIN for voting
          */
         static void on( const voteproducer& vp ) {
            ...
            //validate input
            if ( vp.proxy ) {
               ultrainio_assert( vp.producers.size() == 0, "cannot vote for producers and proxy at same time" );
               require_recipient( vp.proxy );
            } else {
               ultrainio_assert( vp.producers.size() <= 30, "attempt to vote for too many producers" );
               for( size_t i = 1; i < vp.producers.size(); ++i ) {
                  ultrainio_assert( vp.producers[i-1] < vp.producers[i], "producer votes must be unique and sorted" );
               }
            }
            ...
            ultrainio_assert( voter != voters_tbl.end() && ( 0 < voter->staked.quantity || ( voter->is_proxy && 0 < voter->proxied_votes ) ), "no stake to vote" );
            if ( voter->is_proxy ) {
               ultrainio_assert( vp.proxy == 0 , "account registered as a proxy is not allowed to use a proxy" );
            }
            ...
            if( voter->proxy ) {
               ...
               ultrainio_assert( old_proxy != voters_tbl.end(), "old proxy not found" ); //data corruption
            ...
            if ( vp.proxy ) {
               ...
               ultrainio_assert( new_proxy != voters_tbl.end() && new_proxy->is_proxy, "proxy not found" );
            ...
            for ( auto it = elected.begin(); it != end_it; ++it ) {
               auto prod = producers_tbl.find( *it );
               ultrainio_assert( prod != producers_tbl.end(), "producer is not registered" );
               if ( vp.proxy == 0 ) { //direct voting, in case of proxy voting update total_votes even for inactive producers
                  ultrainio_assert( prod->active(), "producer is not currently registered" );
               }
               producers_tbl.modify( prod, 0, [&]( auto& pi ) { pi.total_votes += votes; } );
            }
         ...
         ACTION( SystemAccount, regproxy ) {
            ...
            ULTRAINLIB_SERIALIZE( regproxy, (proxy) )
         };
         ...
         static void on( const regproxy& reg ) {
            ...
            if ( proxy != voters_tbl.end() ) {
               ultrainio_assert( proxy->is_proxy == 0, "account is already a proxy" );
               ultrainio_assert( proxy->proxy == 0, "account that uses a proxy is not allowed to become a proxy" );
         ...
         ACTION( SystemAccount, unregproxy ) {
            ...
            ULTRAINLIB_SERIALIZE( unregproxy, (proxy) )
         };
         ...
         static void on( const unregproxy& reg ) {
            ...
            voters_table voters_tbl( SystemAccount, SystemAccount );
            auto proxy = voters_tbl.find( reg.proxy );
            ultrainio_assert( proxy != voters_tbl.end(), "proxy not found" );
            ultrainio_assert( proxy->is_proxy == 1, "account is not a proxy" );
