...
namespace ultrainio {
   ...
   void set_blockchain_parameters(const ultrainio::blockchain_parameters& params) {
      char buf[sizeof(ultrainio::blockchain_parameters)];
      ultrainio::datastream<char *> ds( buf, sizeof(buf) );
      ds << params;
      set_blockchain_parameters_packed( buf, ds.tellp() );
   }

   void get_blockchain_parameters(ultrainio::blockchain_parameters& params) {
      char buf[sizeof(ultrainio::blockchain_parameters)];
      size_t size = get_blockchain_parameters_packed( buf, sizeof(buf) );
      ultrainio_assert( size <= sizeof(buf), "buffer is too small" );
      ultrainio::datastream<const char*> ds( buf, size_t(size) );
      ds >> params;
   }
   ...
   */
   ...
   private:
      ...
      public:
         ...
         {
            ultrainio_assert(_offset == _heap_size, "malloc_from_freed was designed to only be called after _heap was completely allocated");
...
{
   return ultrainio::memory_heap.malloc(size);
}

void* calloc(size_t count, size_t size)
{
   void* ptr = ultrainio::memory_heap.malloc(count*size);
...
{
   return ultrainio::memory_heap.realloc(ptr, size);
}

void free(void* ptr)
{
   return ultrainio::memory_heap.free(ptr);
}
