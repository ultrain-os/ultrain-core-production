...
namespace ultrainio {
   ...
   #define S(P,X) ::ultrainio::string_to_symbol(P,#X)
   ...
   struct symbol_type {
      ...
      ULTRAINLIB_SERIALIZE( symbol_type, (value) )
   };
   ...
   struct extended_symbol : public symbol_type
   {
      ...
      ULTRAINLIB_SERIALIZE( extended_symbol, (value)(contract) )
   };
   ...
   struct asset {
      ...
      explicit asset( int64_t a = 0, symbol_name s = S(4,ULTRAIN))
      :amount(a),symbol{s}
      {
         ultrainio_assert( is_amount_within_range(), "magnitude of asset amount must be less than 2^62" );
         ultrainio_assert( symbol.is_valid(),        "invalid symbol name" );
      }
      ...
      asset& operator-=( const asset& a ) {
         ultrainio_assert( a.symbol == symbol, "attempt to subtract asset with different symbol" );
         amount -= a.amount;
         ultrainio_assert( -max_amount <= amount, "subtraction underflow" );
         ultrainio_assert( amount <= max_amount,  "subtraction overflow" );
         return *this;
      }

      asset& operator+=( const asset& a ) {
         ultrainio_assert( a.symbol == symbol, "attempt to add asset with different symbol" );
         amount += a.amount;
         ultrainio_assert( -max_amount <= amount, "addition underflow" );
         ultrainio_assert( amount <= max_amount,  "addition overflow" );
         return *this;
      }
      ...
      asset& operator*=( int64_t a ) {
         ultrainio_assert( a == 0 || (amount * a) / a == amount, "multiplication overflow or underflow" );
         ultrainio_assert( -max_amount <= amount, "multiplication underflow" );
         ultrainio_assert( amount <= max_amount,  "multiplication overflow" );
         amount *= a;
         return *this;
      }
      ...
      friend int64_t operator/( const asset& a, const asset& b ) {
         ultrainio_assert( a.symbol == b.symbol, "comparison of assets with different symbols is not allowed" );
      ...
      ULTRAINLIB_SERIALIZE( asset, (amount)(symbol) )
   };
   ...
   struct extended_asset : public asset {
      ...
      friend extended_asset operator - ( const extended_asset& a, const extended_asset& b ) {
         ultrainio_assert( a.contract == b.contract, "type mismatch" );
      ...
      ULTRAINLIB_SERIALIZE( extended_asset, (amount)(symbol)(contract) )
   };
