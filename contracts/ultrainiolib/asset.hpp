...
#include <ultrainiolib/symbol.hpp>
#include <tuple>
#include <limits>
   ...
   */
   ...
   struct asset {
      ...
      asset& operator*=( int64_t a ) {
         ...
         ultrainio_assert( tmp <= max_amount, "multiplication overflow" );
         ultrainio_assert( tmp >= -max_amount, "multiplication underflow" );
         amount = (int64_t)tmp;
         return *this;
      }
       ...
       */
      asset& operator/=( int64_t a ) {
         ultrainio_assert( a != 0, "divide by zero" );
         ultrainio_assert( !(amount == std::numeric_limits<int64_t>::min() && a == -1), "signed division overflow" );
         amount /= a;
         return *this;
      }
      ...
      friend int64_t operator/( const asset& a, const asset& b ) {
         ultrainio_assert( b.amount != 0, "divide by zero");
