...
namespace ultrainio {
   ...
   public:
      ...
      ULTRAINLIB_SERIALIZE_DERIVED( transaction, transaction_header, (context_free_actions)(actions)(transaction_extensions) )
   };
   ...
   struct onerror {
      ...
      ULTRAINLIB_SERIALIZE( onerror, (sender_id)(sent_trx) )
   };
   ...
   inline action get_action( uint32_t type, uint32_t index ) {
      ...
      ultrainio_assert( s > 0, "get_action size failed" );
      size_t size = static_cast<size_t>(s);
      char* buffer = (char*)( max_stack_buffer_size < size ? malloc(size) : alloca(size) );
      auto size2 = ::get_action( type, index, buffer, size );
      ultrainio_assert( size == static_cast<size_t>(size2), "get_action failed" );
      return ultrainio::unpack<ultrainio::action>( buffer, size );
   }
