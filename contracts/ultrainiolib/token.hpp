...
namespace ultrainio {
  ...
  */
  ...
  struct token {
    /**
    * Type of the currency (e.g. ultrain) represented as an unsigned 64 bit integer
    ...
    */
    ...
    token( const asset& a ):quantity(NumberType(a.amount)) {
       ultrainio_assert( a.symbol == Symbol, "attempt to construct token from asset with different symbol" );
       ultrainio_assert( 0 <= a.amount, "attemt to convert asset with negative value to token" );
    }
    ...
    */
    token& operator-=( const token& a ) {
      ultrainio_assert( quantity >= a.quantity, "integer underflow subtracting token balance" );
      quantity -= a.quantity;
      return *this;
    }
    ...
    */
    token& operator+=( const token& a ) {
      ultrainio_assert( quantity + a.quantity >= a.quantity, "integer overflow adding token balance" );
      quantity += a.quantity;
      return *this;
    }
    ...
    */
    token& operator*=( uint64_t a ) {
      ultrainio_assert( a == 0 || (quantity * a) / a == quantity, "integer overflow multiplying token balance" );
      quantity *= a;
      return *this;
    }
  ...
  *  Example:
  *  @code
  *  typedef ultrainio::token<uint64_t, N(MyBaseToken)> MyBaseToken;
  *  typedef ultrainio::token<uint64_t, N(MyQuoteToken)> MyQuoteToken;
  ...
  struct price
  {
    ...
    */
    price( BaseToken base, QuoteToken quote ) {
      ultrainio_assert( base  >= BaseToken(1ul), "invalid price" );
      ultrainio_assert( quote >= QuoteToken(1ul), "invalid price" );
    ...
    */
    ...
    ULTRAINLIB_SERIALIZE( price, (base_per_quote) )
  private:
    /**
