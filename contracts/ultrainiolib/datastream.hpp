   ...
   public:
      ...
      */
      inline bool read( char* d, size_t s ) {
        ultrainio_assert( size_t(_end - _pos) >= (size_t)s, "read" );
        memcpy( d, _pos, s );
        _pos += s;
        return true;
      }
      ...
      */
      inline bool write( const char* d, size_t s ) {
        ultrainio_assert( _end - _pos >= (int32_t)s, "write" );
      ...
      */
      inline bool put(char c) {
        ultrainio_assert( _pos < _end, "put" );
        *_pos = c;
        ++_pos;
        return true;
      }
      ...
      */
      ...
      {
        ultrainio_assert( _pos < _end, "get" );
        c = *_pos;
        ++_pos;
        return true;
      }
...
DataStream& operator >> ( DataStream& ds, T (&v)[N] ) {
   unsigned_int s;
   ds >> s;
   ultrainio_assert( N == s.value, "T[] size and unpacked size don't match");
   for( uint32_t i = 0; i < N; ++i )
      ds >> v[i];
   return ds;
}
