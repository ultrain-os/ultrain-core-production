...
class dice : public ultrainio::contract {
   public:
      ...
      void canceloffer( const checksum256& commitment ) {
         ...
         ultrainio_assert( offer_itr != idx.end(), "offer does not exists" );
         ultrainio_assert( offer_itr->gameid == 0, "unable to cancel offer" );
      ...
      void reveal( const checksum256& commitment, const checksum256& source ) {
         ...
         ultrainio_assert(curr_revealer_offer != idx.end(), "offer not found");
         ultrainio_assert(curr_revealer_offer->gameid > 0, "unable to reveal");
         ...
         ultrainio_assert( is_zero(curr_reveal.reveal) == true, "player already revealed");
      ...
      void claimexpired( const uint64_t gameid ) {
         ...
         if( !is_zero(game_itr->player1.reveal) ) {
            ultrainio_assert( is_zero(game_itr->player2.reveal), "game error");
            pay_and_clean(*game_itr, *player1_offer, *player2_offer);
         } else {
            ultrainio_assert( is_zero(game_itr->player1.reveal), "game error");
   ...
   private:
      ...
      struct player {
         ...
         ULTRAINLIB_SERIALIZE( player, (commitment)(reveal) )
      };
      ...
      struct game {
         ...
         ULTRAINLIB_SERIALIZE( game, (id)(bet)(deadline)(player1)(player2) )
      };
      ...
      struct global_dice {
         ...
         ULTRAINLIB_SERIALIZE( global_dice, (id)(nextgameid) )
      };

      typedef ultrainio::multi_index< N(global), global_dice> global_dice_index;
...
ULTRAINIO_ABI( dice, (offerbet)(canceloffer)(reveal)(claimexpired)(deposit)(withdraw) )
