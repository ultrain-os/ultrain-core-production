...
void tic_tac_toe::create(const account_name& challenger, const account_name& host) {
   require_auth(host);
   ultrainio_assert(challenger != host, "challenger shouldn't be the same as host");
...
void tic_tac_toe::move(const account_name& challenger, const account_name& host, const account_name& by, const uint16_t& row, const uint16_t& column ) {
   ...
   ultrainio_assert(by == itr->turn, "it's not your turn yet!");


   // Check if user makes a valid movement
   ultrainio_assert(is_valid_movement(row, column, itr->board), "not a valid movement!");
...
ULTRAINIO_ABI( tic_tac_toe, (create)(restart)(close)(move))
