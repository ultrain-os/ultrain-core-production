...
namespace identity_test {
   ...
   class contract : public ultrainio::contract {
      public:
         ...
         void getowner( const uint64_t identity ) {
            ...
            result_table( code, 0 ).set( owner ); //use scope = 0 for simplicity
         }
         ...
         void getidentity( const account_name account ) {
            ...
            result_table( code, 0 ).set(idnt ); //use scope = 0 for simplicity
         }
   };

} /// namespace identity

ULTRAINIO_ABI( identity_test::contract, (getowner)(getidentity) );
