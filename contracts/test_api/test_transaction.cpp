...
void copy_data(char* data, size_t data_len, ultrainio::vector<char>& data_out) {
...
void test_transaction::send_action() {
   ...
   action act(ultrainio::vector<permission_level>{{N(testapi), N(active)}}, test_action);
   act.send();
}
...
void test_transaction::send_action_large() {
   ...
   act.send();
   ultrainio_assert(false, "send_message_large() should've thrown an error");
}
...
void test_transaction::test_tapos_block_prefix() {
   ...
   ultrainio_assert( tbp == tapos_block_prefix(), "tapos_block_prefix does not match" );
}
...
void test_transaction::test_tapos_block_num() {
   ...
   ultrainio_assert( tbn == tapos_block_num(), "tapos_block_num does not match" );
}
...
void test_transaction::assert_false_error_handler(const ultrainio::deferred_transaction& dtrx) {
   auto onerror_action = ultrainio::get_action(1, 0);
   ultrainio_assert( onerror_action.authorization.at(0).actor == dtrx.actions.at(0).account,
...
void test_transaction::send_transaction_large(uint64_t receiver, uint64_t, uint64_t) {
   ...
   ultrainio_assert(false, "send_transaction_large() should've thrown an error");
}
...
void test_transaction::send_transaction_expiring_late(uint64_t receiver, uint64_t, uint64_t) {
   ...
   ultrainio_assert(false, "send_transaction_expiring_late() should've thrown an error");
}
...
void test_transaction::send_cf_action_fail() {
   ...
   act.send_context_free();
   ultrainio_assert(false, "send_cfa_action_fail() should've thrown an error");
}
...
void test_transaction::read_inline_action() {
   ...
   ultrainio_assert(res == -1, "get_action error 0");
   ...
   ultrainio_assert(res != -1, "get_action error");
   ...
   ultrainio_assert(dres.a == 1 && dres.b == 2 && dres.c == 3, "data_as error");

   res = get_action( 3, 1, buffer, 64);
   ultrainio_assert(res == -1, "get_action error -1");
}
