...
void test_transaction::test_read_transaction() {
   ...
   ultrainio_assert( size == read, "read_transaction failed");
   sha256(buf, read, &h);
   printhex( &h, sizeof(h) );
}
...
void test_transaction::cancel_deferred_transaction_success() {
   ...
   ultrainio_assert( (bool)r, "transaction was not found" );
}

void test_transaction::cancel_deferred_transaction_not_found() {
   using namespace ultrainio;
   auto r = cancel_deferred( 0xffffffffffffffff ); //use the same id (0) as in send_deferred_transaction
   ultrainio_assert( !r, "transaction was canceled, whild should not be found" );
}
...
void test_transaction::new_feature() {
   ultrainio_assert(false == is_feature_active((int64_t)N(newfeature)), "we should not have new features unless hardfork");
}
