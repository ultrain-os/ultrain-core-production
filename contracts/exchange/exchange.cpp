...
namespace ultrainio {
   ...
   void exchange::withdraw( account_name from, extended_asset quantity ) {
      require_auth( from );
      ultrainio_assert( quantity.is_valid(), "invalid quantity" );
      ultrainio_assert( quantity.amount >= 0, "cannot withdraw negative balance" ); // Redundant? inline_transfer will fail if quantity is not positive.
   ...
   void exchange::on( const trade& t ) {
      ...
      ultrainio_assert( t.sell.is_valid(), "invalid sell amount" );
      ultrainio_assert( t.sell.amount > 0, "sell amount must be positive" );
      ultrainio_assert( t.min_receive.is_valid(), "invalid min receive amount" );
      ultrainio_assert( t.min_receive.amount >= 0, "min receive amount cannot be negative" );

      auto receive_symbol = t.min_receive.get_extended_symbol();
      ultrainio_assert( t.sell.get_extended_symbol() != receive_symbol, "invalid conversion" );
      ...
      if( t.min_receive.amount != 0 ) {
         ultrainio_assert( t.min_receive.amount <= output.amount, "unable to fill" );
      }
   ...
   void exchange::on( const upmargin& b ) {
      ...
      ultrainio_assert( b.delta_borrow.is_valid(), "invalid borrow delta" );
      ultrainio_assert( b.delta_collateral.is_valid(), "invalid collateral delta" );

      market_state market( _this_contract, b.market, _accounts );

      ultrainio_assert( b.delta_borrow.amount != 0 || b.delta_collateral.amount != 0, "no effect" );
      ultrainio_assert( b.delta_borrow.get_extended_symbol() != b.delta_collateral.get_extended_symbol(), "invalid args" );
      ultrainio_assert( market.exstate.base.balance.get_extended_symbol() == b.delta_borrow.get_extended_symbol() ||
                    market.exstate.quote.balance.get_extended_symbol() == b.delta_borrow.get_extended_symbol(),
                    "invalid asset for market" );
      ultrainio_assert( market.exstate.base.balance.get_extended_symbol() == b.delta_collateral.get_extended_symbol() ||
   ...
   void exchange::on( const covermargin& c ) {
      ...
      ultrainio_assert( c.cover_amount.is_valid(), "invalid cover amount" );
      ultrainio_assert( c.cover_amount.amount > 0, "cover amount must be positive" );
   ...
   void exchange::createx( account_name    creator,
      ...
      ultrainio_assert( initial_supply.is_valid(), "invalid initial supply" );
      ultrainio_assert( initial_supply.amount > 0, "initial supply must be positive" );
      ultrainio_assert( base_deposit.is_valid(), "invalid base deposit" );
      ultrainio_assert( base_deposit.amount > 0, "base deposit must be positive" );
      ultrainio_assert( quote_deposit.is_valid(), "invalid quote deposit" );
      ultrainio_assert( quote_deposit.amount > 0, "quote deposit must be positive" );
      ultrainio_assert( base_deposit.get_extended_symbol() != quote_deposit.get_extended_symbol(),
   ...
   void exchange::lend( account_name lender, symbol_type market, extended_asset quantity ) {
      require_auth( lender );
      ultrainio_assert( quantity.is_valid(), "invalid quantity" );
      ultrainio_assert( quantity.amount > 0, "must lend a positive amount" );
   ...
   void exchange::unlend( account_name lender, symbol_type market, double interest_shares, extended_symbol interest_symbol ) {
      require_auth( lender );
      ultrainio_assert( interest_shares > 0, "must unlend a positive amount" );
   ...
   void exchange::on( const currency::transfer& t, account_name code ) {
      ...
      if( t.to == _this_contract ) {
         ...
         ultrainio_assert( a.is_valid(), "invalid quantity in transfer" );
         ultrainio_assert( a.amount != 0, "zero quantity is disallowed in transfer");
         ultrainio_assert( a.amount > 0 || t.memo == "withdraw", "withdrew tokens without withdraw in memo");
         ultrainio_assert( a.amount < 0 || t.memo == "deposit", "received tokens without deposit in memo" );
