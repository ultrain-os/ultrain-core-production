...
void ultrainio_assert( bool test, const string& msg ) {
...
exchange_state convert( const exchange_state& current,
  ...
  ultrainio_assert( min_output.symbol != input.symbol, "cannot convert" );
  ...
  if( input.symbol != exchange_symbol ) {
     ...
     else ultrainio_assert( false, "invalid symbol" );
  } else {
