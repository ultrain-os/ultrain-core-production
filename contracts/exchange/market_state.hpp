...
namespace ultrainio {
   ...
   struct margin_position {
      ...
      ULTRAINLIB_SERIALIZE( margin_position, (owner)(borrowed)(collateral)(call_price) )
   };

   typedef ultrainio::multi_index<N(margins), margin_position,
           indexed_by<N(callprice), ultrainio::const_mem_fun<margin_position, uint64_t, &margin_position::get_call> >
   > margins;
   ...
   struct loan_position {
      ...
      ULTRAINLIB_SERIALIZE( loan_position, (owner)(interest_shares) )
   };

   typedef ultrainio::multi_index<N(loans), loan_position> loans;
