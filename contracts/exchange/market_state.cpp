...
namespace ultrainio {
   ...
   {
      ultrainio_assert( market_state_itr != market_table.end(), "unknown market" );
   ...
   void market_state::margin_call( exchange_state::connector& c, margins& marginstable ) {
      ...
      ultrainio_assert( receipt.amount >= pos->borrowed.amount, "programmer error: insufficient collateral to cover" );/// VERY BAD, SHOULD NOT HAPPEN
   ...
   void market_state::lend( account_name lender, const extended_asset& quantity ) {
      ...
      else ultrainio_assert( false, "unable to lend to this market" );
   }

   void market_state::unlend( account_name lender, double ishares, const extended_symbol& sym ) {
      ultrainio_assert( ishares > 0, "cannot unlend negative balance" );
   ...
   void market_state::cover_margin( account_name borrower, const extended_asset& cover_amount ) {
      if( cover_amount.get_extended_symbol() == exstate.base.balance.get_extended_symbol() ) {
         ...
         ultrainio_assert( false, "invalid debt asset" );
      }
   }
                                    ...
                                    const extended_asset& cover_amount )
   {
      ...
      ultrainio_assert( existing != m.end(), "no known margin position" );
      ultrainio_assert( existing->borrowed.amount >= cover_amount.amount, "attempt to cover more than user has" );
      ...
      ultrainio_assert( debpaid.amount >= cover_amount.amount, "unable to cover debt" );
                                     ...
                                     const extended_asset& delta_debt, const extended_asset& delta_col )
   {
      ...
      if( existing == m.end() ) {
         ...
         if( existing->borrowed.amount == -delta_debt.amount ) {
            ultrainio_assert( existing->collateral.amount == -delta_col.amount, "user failed to claim all collateral" );
      ...
      ultrainio_assert( c.peer_margin.total_lent.amount <= c.peer_margin.total_lendable.amount, "insufficient funds availalbe to borrow" );

      if( existing != m.end() ) {
         if( existing->call_price < c.peer_margin.least_collateralized )
            c.peer_margin.least_collateralized = existing->call_price;

         ultrainio_assert( !exstate.requires_margin_call( c ), "this update would trigger a margin call" );
      } else {
         c.peer_margin.least_collateralized = std::numeric_limits<double>::max();
      }
