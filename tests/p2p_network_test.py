...
# ultrainio should have the same key as defproducera
ultrainio = copy.copy(defproduceraAccount)
ultrainio.name = "ultrainio"
...
trans=node0.publishContract(ultrainio.name, wastFile, abiFile, waitForTransBlock=True)
if trans is None:
...
try:
    ...
    for cmdInd in range(maxIndex):
        (transIdList, checkacct, expBal, errmsg) = module.execute(cmdInd, node0, testeraAccount, ultrainio)
