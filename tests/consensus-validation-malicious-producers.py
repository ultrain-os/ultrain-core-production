...
private-key = ["UTR6MRyAjQq8ud7hVNYcfnVPJqcVpscN5So8BhtHuGYqET5GDW5CV","5KQwrPbwdL6PhXujxW37FSSQZ1JiwsST4cqQzDeyXtP79zkvFD3"]
producer-name = initu
plugin = ultrainio::producer_plugin
plugin = ultrainio::chain_api_plugin
plugin = ultrainio::account_history_plugin
...
def stageScenario(stagedNodeInfos):
    ...
    for stagedNodeInfo in stagedNodeInfos:
        configPath=os.path.join(stagingDir, "etc/ultrainio/node_%02d" % (count))
...
def myTest(transWillEnterBlock):
    ...
    try:
        ...
        if cluster.launch(pnodes, total_nodes, topo, delay) is False:
            error("Failed to stand up ultrain cluster.")
            return False
        ...
        if walletMgr.launch() is False:
            error("Failed to stand up ultrain walletd.")
            return False
