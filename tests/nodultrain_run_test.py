...
###############################################################
# nodultrain_run_test
# --dump-error-details <Upon error print etc/ultrainio/node_*/config.ini and var/lib/node_*/stderr.log to stdout>
...
WalletdName="kultraind"
ClientName="clultrain"
...
try:
    ...
    testWallet=walletMgr.create(testWalletName, [cluster.ultrainioAccount,cluster.defproduceraAccount,cluster.defproducerbAccount])
    if testWallet is None:
        cmdError("ultrain wallet create")
    ...
    # create accounts via ultrainio as otherwise a bid is needed 
    Print("Create new account %s via %s" % (testeraAccount.name, cluster.ultrainioAccount.name))
    transId=node.createInitializeAccount(testeraAccount, cluster.ultrainioAccount, stakedDeposit=0, waitForTransBlock=False)
    if transId is None:
        cmdError("%s create account" % (testeraAccount.name))
        errorExit("Failed to create account %s" % (testeraAccount.name))

    Print("Create new account %s via %s" % (currencyAccount.name, cluster.ultrainioAccount.name))
    transId=node.createInitializeAccount(currencyAccount, cluster.ultrainioAccount, stakedDeposit=5000)
    if transId is None:
        cmdError("%s create account" % (ClientName))
        errorExit("Failed to create account %s" % (currencyAccount.name))

    Print("Create new account %s via %s" % (exchangeAccount.name, cluster.ultrainioAccount.name))
    transId=node.createInitializeAccount(exchangeAccount, cluster.ultrainioAccount, waitForTransBlock=True)
    if transId is None:
        cmdError("%s create account" % (ClientName))
    ...
    contractDir="contracts/ultrainio.token"
    wastFile="contracts/ultrainio.token/ultrainio.token.wast"
    abiFile="contracts/ultrainio.token/ultrainio.token.abi"
    ...
    if not enableMongo:
        ...
        if abiName != "transfer" or abiActionName != "transfer" or abiType != "transfer":
            errorExit("FAILURE - get UTR account failed", raw=True)
