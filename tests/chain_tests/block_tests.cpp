...
uint32_t calc_exp_last_irr_block_num(const base_tester& chain, const uint32_t& head_block_num) {
   ...
   const auto max_reversible_rounds = ULTRAIN_PERCENT(producers_size, config::percent_100 - config::irreversible_threshold_percent);
...
{ try {
      ...
      chain.push_action(name("ultrainio"), name("updateauth"), name("tester"), fc::mutable_variant_object()
              ("account", "tester")
              ("permission", "first")
              ("parent", "active")
