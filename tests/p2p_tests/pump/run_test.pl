...
my $ultrain_home = defined $ENV{ULTRAIN_HOME} ? $ENV{ULTRAIN_HOME} : getcwd;
my $ultraind = $ultrain_home . "/programs/ultraind/ultraind";
my $ultrainc = $ultrain_home . "/programs/ultrainc/ultrainc";

my $nodes = defined $ENV{ULTRAIN_TEST_RING} ? $ENV{ULTRAIN_TEST_RING} : "1";
my $pnodes = defined $ENV{ULTRAIN_TEST_PRODUCERS} ? $ENV{ULTRAIN_TEST_PRODUCERS} : "1";

my $prods = 21;
my $genesis = "$ultrain_home/genesis.json";
...
if (!GetOptions("nodes=i" => \$nodes,
    ...
    print "where:\n";
    print "--nodes=n (default = 1) sets the number of ultraind instances to launch\n";
...
sub write_config {
    ...
    if (defined $producer) {
        ...
        print $cfg "private-key = [\"ULTRAIN6MRyAjQq8ud7hVNYcfnVPJqcVpscN5So8BhtHuGYqET5GDW5CV\",\"5KQwrPbwdL6PhXujxW37FSSQZ1JiwsST4cqQzDeyXtP79zkvFD3\"]\n";

        print $cfg "plugin = ultrainio::producer_plugin\n";
        print $cfg "plugin = ultrainio::chain_api_plugin\n";
...
sub perform_work {
    ...
    if ($mode == 0) {
        ...
        my $counter = 0;
        while (time () < $stoptime) {
            `$ultrainc transfer ultrain inita 10 >> ultrainc.out 2>> ultrainc.err`;
            $counter++;
            if ($counter % 1000 == 0) {
