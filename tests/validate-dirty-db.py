...
###############################################################
# Test for validating the dirty db flag sticks repeated nodultrain restart attempts
###############################################################
...
def runNodultrainAndGetOutput(myTimeout=3):
    """Startup nodultrain, wait for timeout (before forced shutdown) and collect output. Stdout, stderr and return code are returned in a dictionary."""
    Print("Launching nodultrain process.")
    cmd="programs/nodultrain/nodultrain --config-dir etc/ultrainio/node_bios --data-dir var/lib/node_bios --verbose-http-errors --http-validate-host=false"
    Print("cmd: %s" % (cmd))
    proc=subprocess.Popen(cmd.split(), stdout=subprocess.PIPE, stderr=subprocess.PIPE)
    if debug: Print("Nodultrain process launched.")

    output={}
    try:
        if debug: Print("Setting nodultrain process timeout.")
        outs,errs = proc.communicate(timeout=myTimeout)
        if debug: Print("Nodultrain process has exited.")
        output["stdout"] = outs.decode("utf-8")
        output["stderr"] = errs.decode("utf-8")
        output["returncode"] = proc.returncode
    except (subprocess.TimeoutExpired) as _:
        Print("ERROR: Nodultrain is running beyond the defined wait time. Hard killing nodultrain instance.")
...
try:
    ...
    Print("Restart nodultrain repeatedly to ensure dirty database flag sticks.")
    timeout=3
    
    for i in range(1,4):
        Print("Attempt %d." % (i))
        ret = runNodultrainAndGetOutput(timeout)
        assert(ret)
