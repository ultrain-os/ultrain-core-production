...
#include <ultrain/chain/account_object.hpp>
#include <ultrain/chain/producer_object.hpp>
#include <ultrain/chain/authority_checker.hpp>
#include <ultrain/producer_plugin/producer_plugin.hpp>

#include <ultrain/utilities/tempdir.hpp>

#include <ultrain/native_contract/native_contract_chain_initializer.hpp>
#include <ultrain/native_contract/native_contract_chain_administrator.hpp>
#include <ultrain/native_contract/objects.hpp>
...
uint32_t ULTRAIN_TESTING_GENESIS_TIMESTAMP = 1431700005;

namespace ultrainio { namespace chain {

testing_fixture::testing_fixture() {
   default_genesis_state.initial_timestamp = fc::time_point_sec(ULTRAIN_TESTING_GENESIS_TIMESTAMP);
...
private_key_type testing_fixture::get_private_key(const public_key_type& public_key) const {
   ...
   ULTRAIN_ASSERT(itr != key_ring.end(), missing_key_exception,
...
testing_blockchain::testing_blockchain(chainbase::database& db, fork_database& fork_db, block_log& blocklog,
   ...
   : chain_controller(db, fork_db, blocklog, initializer, native_contract::make_administrator(),
                      ::ultrainio::chain_plugin::default_transaction_execution_time * 1000,
                      ::ultrainio::chain_plugin::default_received_block_transaction_execution_time * 1000,
                      ::ultrainio::chain_plugin::default_create_block_transaction_execution_time * 1000,
...
types::asset testing_blockchain::get_liquid_balance(const types::account_name& account) {
   return get_database().get<balance_object, ultrainio::chain::by_owner_name>(account).balance;
}

types::asset testing_blockchain::get_staked_balance(const types::account_name& account) {
   return get_database().get<staked_balance_object, ultrainio::chain::by_owner_name>(account).staked_balance;
}

types::asset testing_blockchain::get_unstaking_balance(const types::account_name& account) {
   return get_database().get<staked_balance_object, ultrainio::chain::by_owner_name>(account).unstaking_balance;
}
