 ...
 *
 * @code{.cpp}
 * // Link alice's "money" authority with ultrainio::transfer
 * Link_Authority(chain, alice, "money", ultrain, "transfer");
 * // Set alice's "native" authority as default for ultrain contract
 * Link_Authority(chain, alice, "money", ultrain);
 * @endcode
 */
 ...
 *
 * @code{.cpp}
 * // Unlink alice's authority for ultrainio::transfer
 * Unlink_Authority(chain, alice, ultrain, "transfer");
 * // Unset alice's default authority for ultrain contract
 * Unlink_Authority(chain, alice, ultrain);
 * @endcode
 */
 ...
 * @code{.cpp}
 * // Send 10 ULTRAIN from alice to bob
 * Transfer_Asset(chain, alice, bob, asset(10));
 *
 * // Send 10 ULTRAIN from alice to bob with memo "Thanks for all the fish!"
 ...
 * @code{.cpp}
 * // Convert 10 of bob's ULTRAIN from liquid to staked
 * Stake_Asset(chain, bob, asset(10).amount);
 *
 * // Stake and transfer 10 ULTRAIN from alice to bob (alice pays liquid ULTRAIN and bob receives stake)
 ...
 * @code{.cpp}
 * // Begin unstaking 10 of bob's ULTRAIN
...
/**
 * @brief Shorthand way to claim unstaked ULTRAIN as liquid
 *
 * Use Finish_Unstake_Asset to liquidate unstaked funds:
 * @code{.cpp}
 * // Reclaim as liquid 10 of bob's unstaked ULTRAIN
 * Unstake_Asset(chain, bob, asset(10).amount);
 * @endcode
 */
...
{ \
   ultrainio::chain::signed_transaction trx; \
   if (std::string(#stakeholder) != std::string(#proxy)) \
      transaction_emplace_message(trx, config::ultrain_contract_name, \
