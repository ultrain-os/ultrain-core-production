...
killAll()
{
  programs/ultrainio-launcher/ultrainio-launcher -k 15
}

cleanup()
{
    rm -rf etc/ultrainio/node_*
    rm -rf var/lib/node_*
}
...
getHeadBlockNum()
{
  INFO="$(programs/clultrain/clultrain get info)"
  verifyErrorCode "clultrain get info"
...
# stand up nodultrain cluster
launcherOpts="-p $pnodes -n $total_nodes -s $topo -d $delay"
echo Launcher options: --nodultrain \"--plugin ultrainio::wallet_api_plugin\" $launcherOpts
programs/ultrainio-launcher/ultrainio-launcher --nodultrain "--plugin ultrainio::wallet_api_plugin" $launcherOpts
sleep 7
...
while [ $port2  -ne $endport ]; do
    ...
    TRANS_INFO="$(programs/clultrain/clultrain --port $port2 get block 1)"
    verifyErrorCode "clultrain get block"
    port2=`expr $port2 + 1`
done

# create 3 keys
KEYS="$(programs/clultrain/clultrain create key)"
verifyErrorCode "clultrain create key"
...
PASSWORD_INITA="$(programs/clultrain/clultrain wallet create --name inita)"
verifyErrorCode "clultrain wallet create"
# strip out password from output
PASSWORD_INITA="$(echo "$PASSWORD_INITA" | awk '/PW/ {print $1}')"
# remove leading/trailing quotes
PASSWORD_INITA=${PASSWORD_INITA#\"}
PASSWORD_INITA=${PASSWORD_INITA%\"}
programs/clultrain/clultrain wallet import --name inita --private-key $INITA_PRV_KEY
verifyErrorCode "clultrain wallet import"
programs/clultrain/clultrain wallet import --name inita --private-key $PRV_KEY1
verifyErrorCode "clultrain wallet import"
programs/clultrain/clultrain wallet import --name inita --private-key $PRV_KEY2
verifyErrorCode "clultrain wallet import"
programs/clultrain/clultrain wallet import --name inita --private-key $PRV_KEY3
verifyErrorCode "clultrain wallet import"
...
ACCOUNT_INFO="$(programs/clultrain/clultrain create account inita testera $PUB_KEY1 $PUB_KEY3)"
verifyErrorCode "clultrain create account"
waitForNextBlock
# verify account created
ACCOUNT_INFO="$(programs/clultrain/clultrain get account testera)"
verifyErrorCode "clultrain get account"
...
while [ $port  -ne $endport ]; do
    ...
    TRANSFER_INFO="$(programs/clultrain/clultrain transfer inita testera 975321 "test transfer")"
    verifyErrorCode "clultrain transfer"
   ...
   TRANS_INFO="$(programs/clultrain/clultrain --port $port2 get transaction $TRANS_ID)"
   verifyErrorCode "clultrain get transaction trans_id of <$TRANS_INFO> from node on port $port2"
	port2=`expr $port2 + 1`
    done
