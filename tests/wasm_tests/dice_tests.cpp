...
FC_REFLECT(account_t, (owner)(ultrain_balance)(open_offers)(open_games));
...
BOOST_FIXTURE_TEST_CASE( dice_test, dice_tester ) try {
   ...
   set_code(config::system_account_name, ultrainio_token_wast);
   set_abi(config::system_account_name, ultrainio_token_abi);
   ...
   push_action(N(ultrainio), N(create), N(ultrainio), mvo()
     ("issuer", "ultrainio")
     ("maximum_supply", "1000000000000.0000 UTR")
     ("can_freeze", "0")
     ("can_recall", "0")
     ("can_whitelist", "0")
   );

   push_action(N(ultrainio), N(issue), N(ultrainio), mvo()
     ("to", "ultrainio")
     ("quantity", "1000000.0000 UTR")
     ("memo", "")
   );

   transfer( N(ultrainio), N(alice), "10000.0000 UTR", "", N(ultrainio) );
   transfer( N(ultrainio), N(bob),   "10000.0000 UTR", "", N(ultrainio) );
   transfer( N(ultrainio), N(carol), "10000.0000 UTR", "", N(ultrainio) );
   ...
   // Alice deposits 1000 ULTRAIN
   deposit( N(alice), asset::from_string("1000.0000 UTR")); 
   produce_block();

   BOOST_REQUIRE_EQUAL( balance_of(N(alice)), asset::from_string("1000.0000 UTR"));
   BOOST_REQUIRE_EQUAL( open_games(N(alice)), 0);

   // Alice tries to bet 0 ULTRAIN (fail)
   // secret : 9b886346e1351d4144d0b8392a975612eb0f8b6de7eae1cc9bcc55eb52be343c
   BOOST_CHECK_THROW( offer_bet( N(alice), asset::from_string("0.0000 UTR"), 
      commitment_for("9b886346e1351d4144d0b8392a975612eb0f8b6de7eae1cc9bcc55eb52be343c")
   ), fc::exception);
   
   // Alice bets 10 ULTRAIN (success)
   // secret : 0ba044d2833758ee2c8f24d8a3f70c82c334abe6ce13219a4cf3b862abb03c46
   offer_bet( N(alice), asset::from_string("10.0000 UTR"), 
   ...
   BOOST_CHECK_THROW( offer_bet( N(bob), asset::from_string("10.0000 UTR"),
      commitment_for("0ba044d2833758ee2c8f24d8a3f70c82c334abe6ce13219a4cf3b862abb03c46")
   ), fc::exception);
   produce_block();

   // Alice tries to bet 1000 ULTRAIN (fail)
   // secret : a512f6b1b589a8906d574e9de74a529e504a5c53a760f0991a3e00256c027971
   BOOST_CHECK_THROW( offer_bet( N(alice), asset::from_string("10000.0000 UTR"), 
      commitment_for("a512f6b1b589a8906d574e9de74a529e504a5c53a760f0991a3e00256c027971")
   ), fc::exception);
   produce_block();

   // Bob tries to bet 90 ULTRAIN without deposit
   // secret : 4facfc98932dde46fdc4403125a16337f6879a842a7ff8b0dc8e1ecddd59f3c8
   BOOST_CHECK_THROW( offer_bet( N(bob), asset::from_string("90.0000 UTR"), 
      commitment_for("4facfc98932dde46fdc4403125a16337f6879a842a7ff8b0dc8e1ecddd59f3c8")
   ), fc::exception);
   produce_block();

   // Bob deposits 500 ULTRAIN
   deposit( N(bob), asset::from_string("500.0000 UTR"));
   BOOST_REQUIRE_EQUAL( balance_of(N(bob)), asset::from_string("500.0000 UTR"));

   // Bob bets 11 ULTRAIN (success)
   // secret : eec3272712d974c474a3e7b4028b53081344a5f50008e9ccf918ba0725a8d784
   offer_bet( N(bob), asset::from_string("11.0000 UTR"), 
   ...
   // Carol deposits 300 ULTRAIN
   deposit( N(carol), asset::from_string("300.0000 UTR"));

   // Carol bets 10 ULTRAIN (success)
   // secret : 3efb4bd5e19b780f4980c919330c0306f8157f93db1fc72c7cefec63e0e7f37a
   offer_bet( N(carol), asset::from_string("10.0000 UTR"), 
   ...
   BOOST_REQUIRE_EQUAL( game_bet(1), asset::from_string("10.0000 UTR"));
   ...
   BOOST_REQUIRE_EQUAL( open_games(N(alice)), 0);
   BOOST_REQUIRE_EQUAL( open_offers(N(alice)), 0);
   BOOST_REQUIRE_EQUAL( balance_of(N(alice)), asset::from_string("1010.0000 UTR"));
   
   BOOST_REQUIRE_EQUAL( open_games(N(carol)), 0);
   BOOST_REQUIRE_EQUAL( open_offers(N(carol)), 0);
   BOOST_REQUIRE_EQUAL( balance_of(N(carol)), asset::from_string("290.0000 UTR"));

   // Alice withdraw 1009 ULTRAIN (success)
   withdraw( N(alice), asset::from_string("1009.0000 UTR"));
   BOOST_REQUIRE_EQUAL( balance_of(N(alice)), asset::from_string("1.0000 UTR"));

   BOOST_REQUIRE_EQUAL( 
      get_currency_balance(N(ultrainio), ULTRAIN_SYMBOL, N(alice)),
      asset::from_string("10009.0000 UTR")
   );

   // Alice withdraw 2 ULTRAIN (fail)
   BOOST_CHECK_THROW( withdraw( N(alice), asset::from_string("2.0000 UTR")), 
      fc::exception);

   // Alice withdraw 1 ULTRAIN (success)
   withdraw( N(alice), asset::from_string("1.0000 UTR"));

   BOOST_REQUIRE_EQUAL( 
      get_currency_balance(N(ultrainio), ULTRAIN_SYMBOL, N(alice)),
      asset::from_string("10010.0000 UTR")
   );
