...
class ultrainio_system_tester : public TESTER {
public:

   ultrainio_system_tester() {
      produce_blocks( 2 );
   ...
   asset get_balance( const account_name& act ) {
      //return get_currency_balance( config::system_account_name, symbol(SY(4,ULTRAIN)), act );
      ...
      if (tbl) {
         const auto *obj = db.find<contracts::key_value_object, contracts::by_scope_primary>(boost::make_tuple(tbl->id, symbol(SY(4,ULTRAIN)).value()));
         if (obj) {
      ...
      return asset( result, symbol(SY(4,ULTRAIN)) );
   }
...
inline uint64_t M( const string& ultrain_str ) {
   return asset::from_string( ultrain_str ).amount;
}

BOOST_AUTO_TEST_SUITE(ultrainio_system_tests)

BOOST_FIXTURE_TEST_CASE( stake_unstake, ultrainio_system_tester ) try {
   issue( "alice", "1000.0000 UTR", config::system_account_name );
   BOOST_REQUIRE_EQUAL( asset::from_string("1000.0000 UTR"), get_balance( "alice" ) );

   BOOST_REQUIRE_EQUAL( success(), stake( "alice", "200.0000 UTR", "100.0000 UTR", "500.0000 UTR" ) );

   auto total = get_total_stake( "alice" );
   BOOST_REQUIRE_EQUAL( asset::from_string("200.0000 UTR").amount, total["net_weight"].as_uint64());
   BOOST_REQUIRE_EQUAL( asset::from_string("100.0000 UTR").amount, total["cpu_weight"].as_uint64());
   BOOST_REQUIRE_EQUAL( asset::from_string("500.0000 UTR").amount, total["storage_stake"].as_uint64());
   REQUIRE_MATCHING_OBJECT( voter( "alice", "300.0000 UTR"), get_voter_info( "alice" ) );

   auto bytes = total["storage_bytes"].as_uint64();
   BOOST_REQUIRE_EQUAL( true, 0 < bytes );

   BOOST_REQUIRE_EQUAL( asset::from_string("200.0000 UTR"), get_balance( "alice" ) );

   //unstake
   BOOST_REQUIRE_EQUAL( success(), unstake( "alice", "200.0000 UTR", "100.0000 UTR", bytes ) );
   total = get_total_stake( "alice" );
   BOOST_REQUIRE_EQUAL( asset::from_string("0.0000 UTR").amount, total["net_weight"].as_uint64());
   BOOST_REQUIRE_EQUAL( asset::from_string("0.0000 UTR").amount, total["cpu_weight"].as_uint64());
   BOOST_REQUIRE_EQUAL( asset::from_string("0.0000 UTR").amount, total["storage_stake"].as_uint64());
   BOOST_REQUIRE_EQUAL( 0, total["storage_bytes"].as_uint64());
   REQUIRE_MATCHING_OBJECT( voter( "alice", "0.00 UTR" ), get_voter_info( "alice" ) );
   produce_blocks(1);
   BOOST_REQUIRE_EQUAL( asset::from_string("200.0000 UTR"), get_balance( "alice" ) );
...
BOOST_FIXTURE_TEST_CASE( fail_without_auth, ultrainio_system_tester ) try {
   issue( "alice", "1000.0000 UTR",  config::system_account_name );
   ...
   BOOST_REQUIRE_EQUAL( error("missing authority of alice"),
                        push_action( N(alice), N(delegatebw), mvo()
                                    ("from",     "alice")
                                    ("receiver", "bob")
                                    ("stake_net", "10.0000 UTR")
                                    ("stake_cpu", "10.0000 UTR")
                                    ("stake_storage", "10.0000 UTR"),
                                    false
                        )
   );
   ...
   BOOST_REQUIRE_EQUAL( error("missing authority of alice"),
                        push_action(N(alice), N(undelegatebw), mvo()
                                    ("from",     "alice")
                                    ("receiver", "bob")
                                    ("unstake_net", "200.0000 UTR")
                                    ("unstake_cpu", "100.0000 UTR")
                                    ("unstake_bytes", 0)
                                    ,false
                        )
   );
...
BOOST_FIXTURE_TEST_CASE( stake_negative, ultrainio_system_tester ) try {
   issue( "alice", "1000.0000 UTR",  config::system_account_name );

   BOOST_REQUIRE_EQUAL( error("condition: assertion failed: must stake a positive amount"),
                        stake( "alice", "-0.0001 UTR", "0.0000 UTR", "0.0000 UTR" )
   );

   BOOST_REQUIRE_EQUAL( error("condition: assertion failed: must stake a positive amount"),
                        stake( "alice", "0.0000 UTR", "-0.0001 UTR", "0.0000 UTR" )
   );

   BOOST_REQUIRE_EQUAL( error("condition: assertion failed: must stake a positive amount"),
                        stake( "alice", "00.0000 UTR", "00.0000 UTR", "0.0000 UTR" )
   );

   BOOST_REQUIRE_EQUAL( error("condition: assertion failed: must stake a positive amount"),
                        stake( "alice", "0.0000 UTR", "00.0000 UTR", "0.0000 UTR" )
   );
...
BOOST_FIXTURE_TEST_CASE( unstake_negative, ultrainio_system_tester ) try {
   issue( "alice", "1000.0000 UTR",  config::system_account_name );

   BOOST_REQUIRE_EQUAL( success(), stake( "alice", "bob", "200.0001 UTR", "100.0001 UTR", "300.0000 UTR" ) );

   auto total = get_total_stake( "bob" );
   BOOST_REQUIRE_EQUAL( asset::from_string("200.0001 UTR").amount, total["net_weight"].as_uint64());
   REQUIRE_MATCHING_OBJECT( voter( "alice", "300.0002 UTR" ), get_voter_info( "alice" ) );

   BOOST_REQUIRE_EQUAL( error("condition: assertion failed: must unstake a positive amount"),
                        unstake( "alice", "bob", "-1.0000 UTR", "0.0000 UTR", 0 )
   );

   BOOST_REQUIRE_EQUAL( error("condition: assertion failed: must unstake a positive amount"),
                        unstake( "alice", "bob", "0.0000 UTR", "-1.0000 UTR", 0 )
   );

   //unstake all zeros
   BOOST_REQUIRE_EQUAL( error("condition: assertion failed: must unstake a positive amount"),
                        unstake( "alice", "bob", "0.0000 UTR", "0.0000 UTR", 0 )
   );

} FC_LOG_AND_RETHROW()


BOOST_FIXTURE_TEST_CASE( unstake_more_than_at_stake, ultrainio_system_tester ) try {
   issue( "alice", "1000.0000 UTR",  config::system_account_name );
   BOOST_REQUIRE_EQUAL( success(), stake( "alice", "200.0000 UTR", "100.0000 UTR", "150.0000 UTR" ) );

   auto total = get_total_stake( "alice" );
   BOOST_REQUIRE_EQUAL( asset::from_string("200.0000 UTR").amount, total["net_weight"].as_uint64());
   BOOST_REQUIRE_EQUAL( asset::from_string("100.0000 UTR").amount, total["cpu_weight"].as_uint64());
   BOOST_REQUIRE_EQUAL( asset::from_string("150.0000 UTR").amount, total["storage_stake"].as_uint64());
   auto bytes = total["storage_bytes"].as_uint64();
   BOOST_REQUIRE_EQUAL( true, 0 < bytes );

   BOOST_REQUIRE_EQUAL( asset::from_string("550.0000 UTR"), get_balance( "alice" ) );
   ...
   BOOST_REQUIRE_EQUAL( error("condition: assertion failed: insufficient staked net bandwidth"),
                        unstake( "alice", "200.0001 UTR", "0.0000 UTR", 0 )
   );
   ...
   BOOST_REQUIRE_EQUAL( error("condition: assertion failed: insufficient staked cpu bandwidth"),
                        unstake( "alice", "000.0000 UTR", "100.0001 UTR", 0 )
   );
   ...
   BOOST_REQUIRE_EQUAL( error("condition: assertion failed: insufficient staked storage"),
                        unstake( "alice", "000.0000 UTR", "0.0000 UTR", bytes+1 )
   );
...
BOOST_FIXTURE_TEST_CASE( delegate_to_another_user, ultrainio_system_tester ) try {
   issue( "alice", "1000.0000 UTR",  config::system_account_name );

   BOOST_REQUIRE_EQUAL( success(), stake ( "alice", "bob", "200.0000 UTR", "100.0000 UTR", "80.0000 UTR" ) );

   auto total = get_total_stake( "bob" );
   BOOST_REQUIRE_EQUAL( asset::from_string("200.0000 UTR").amount, total["net_weight"].as_uint64());
   BOOST_REQUIRE_EQUAL( asset::from_string("100.0000 UTR").amount, total["cpu_weight"].as_uint64());
   BOOST_REQUIRE_EQUAL( asset::from_string("80.0000 UTR").amount, total["storage_stake"].as_uint64());
   auto bytes = total["storage_bytes"].as_uint64();
   BOOST_REQUIRE_EQUAL( true, 0 < bytes );
   BOOST_REQUIRE_EQUAL( asset::from_string("620.0000 UTR"), get_balance( "alice" ) );
   ...
   BOOST_REQUIRE_EQUAL( error("condition: assertion failed: unable to find key"),
                        unstake( "bob", "0.0000 UTR", "10.0000 UTR", bytes )
   );

   issue( "carol", "1000.0000 UTR",  config::system_account_name );
   BOOST_REQUIRE_EQUAL( success(), stake( "carol", "bob", "20.0000 UTR", "10.0000 UTR", "8.0000 UTR" ) );
   total = get_total_stake( "bob" );
   BOOST_REQUIRE_EQUAL( asset::from_string("220.0000 UTR").amount, total["net_weight"].as_uint64());
   BOOST_REQUIRE_EQUAL( asset::from_string("110.0000 UTR").amount, total["cpu_weight"].as_uint64());
   BOOST_REQUIRE_EQUAL( asset::from_string("88.0000 UTR").amount, total["storage_stake"].as_uint64());
   auto bytes2 = total["storage_bytes"].as_uint64();
   BOOST_REQUIRE_EQUAL( true, bytes < bytes2 );
   BOOST_REQUIRE_EQUAL( asset::from_string("962.0000 UTR"), get_balance( "carol" ) );
   REQUIRE_MATCHING_OBJECT( voter( "carol", "30.0000 UTR" ), get_voter_info( "carol" ) );

   //alice should not be able to unstake money staked by carol
   BOOST_REQUIRE_EQUAL( error("condition: assertion failed: insufficient staked net bandwidth"),
                        unstake( "alice", "bob", "2001.0000 UTR", "1.0000 UTR", bytes-1 )
   );

   BOOST_REQUIRE_EQUAL( error("condition: assertion failed: insufficient staked cpu bandwidth"),
                        unstake( "alice", "bob", "1.0000 UTR", "101.0000 UTR", bytes-1 )
   );

    BOOST_REQUIRE_EQUAL( error("condition: assertion failed: insufficient staked storage"),
                         unstake( "alice", "bob", "1.0000 UTR", "1.0000 UTR", bytes+1 )
    );
...
BOOST_FIXTURE_TEST_CASE( stake_unstake_separate, ultrainio_system_tester ) try {
   issue( "alice", "1000.0000 UTR",  config::system_account_name );
   BOOST_REQUIRE_EQUAL( asset::from_string("1000.0000 UTR"), get_balance( "alice" ) );

   //everything at once
   BOOST_REQUIRE_EQUAL( success(), stake( "alice", "10.0000 UTR", "20.0000 UTR", "30.0000 UTR" ) );
   auto total = get_total_stake( "alice" );
   BOOST_REQUIRE_EQUAL( asset::from_string("10.0000 UTR").amount, total["net_weight"].as_uint64());
   BOOST_REQUIRE_EQUAL( asset::from_string("20.0000 UTR").amount, total["cpu_weight"].as_uint64());
   BOOST_REQUIRE_EQUAL( asset::from_string("30.0000 UTR").amount, total["storage_stake"].as_uint64());
   auto bytes = total["storage_bytes"].as_uint64();
   BOOST_REQUIRE_EQUAL( true, 0 < bytes );

   //cpu
   BOOST_REQUIRE_EQUAL( success(), stake( "alice", "100.0000 UTR", "0.0000 UTR", "0.0000 UTR" ) );
   total = get_total_stake( "alice" );
   BOOST_REQUIRE_EQUAL( asset::from_string("110.0000 UTR").amount, total["net_weight"].as_uint64());
   BOOST_REQUIRE_EQUAL( asset::from_string("20.0000 UTR").amount, total["cpu_weight"].as_uint64());
   BOOST_REQUIRE_EQUAL( asset::from_string("30.0000 UTR").amount, total["storage_stake"].as_uint64());
   BOOST_REQUIRE_EQUAL( bytes, total["storage_bytes"].as_uint64() );

   //net
   BOOST_REQUIRE_EQUAL( success(), stake( "alice", "0.0000 UTR", "200.0000 UTR", "0.0000 UTR" ) );
   total = get_total_stake( "alice" );
   BOOST_REQUIRE_EQUAL( asset::from_string("110.0000 UTR").amount, total["net_weight"].as_uint64());
   BOOST_REQUIRE_EQUAL( asset::from_string("220.0000 UTR").amount, total["cpu_weight"].as_uint64());
   BOOST_REQUIRE_EQUAL( asset::from_string("30.0000 UTR").amount, total["storage_stake"].as_uint64());
   BOOST_REQUIRE_EQUAL( bytes, total["storage_bytes"].as_uint64() );

   //storage
   BOOST_REQUIRE_EQUAL( success(), stake( "alice", "0.0000 UTR", "0.0000 UTR", "300.0000 UTR" ) );
   total = get_total_stake( "alice" );
   BOOST_REQUIRE_EQUAL( asset::from_string("110.0000 UTR").amount, total["net_weight"].as_uint64());
   BOOST_REQUIRE_EQUAL( asset::from_string("220.0000 UTR").amount, total["cpu_weight"].as_uint64());
   BOOST_REQUIRE_EQUAL( asset::from_string("330.0000 UTR").amount, total["storage_stake"].as_uint64());
   auto bytes2 = total["storage_bytes"].as_uint64();
   BOOST_REQUIRE_EQUAL( true, bytes < bytes2 );

   //unstake cpu
   BOOST_REQUIRE_EQUAL( success(), unstake( "alice", "100.0000 UTR", "0.0000 UTR", 0 ) );
   total = get_total_stake( "alice" );
   BOOST_REQUIRE_EQUAL( asset::from_string("10.0000 UTR").amount, total["net_weight"].as_uint64());
   BOOST_REQUIRE_EQUAL( asset::from_string("220.0000 UTR").amount, total["cpu_weight"].as_uint64());
   BOOST_REQUIRE_EQUAL( asset::from_string("330.0000 UTR").amount, total["storage_stake"].as_uint64());
   ...
   //unstake net
   BOOST_REQUIRE_EQUAL( success(), unstake( "alice", "0.0000 UTR", "200.0000 UTR", 0 ) );
   total = get_total_stake( "alice" );
   BOOST_REQUIRE_EQUAL( asset::from_string("10.0000 UTR").amount, total["net_weight"].as_uint64());
   BOOST_REQUIRE_EQUAL( asset::from_string("20.0000 UTR").amount, total["cpu_weight"].as_uint64());
   BOOST_REQUIRE_EQUAL( asset::from_string("330.0000 UTR").amount, total["storage_stake"].as_uint64());
   ...
   //unstake cpu
   BOOST_REQUIRE_EQUAL( success(), unstake( "alice", "0.0000 UTR", "0.0000 UTR", bytes2 / 2 ) );
   total = get_total_stake( "alice" );
   auto storage_left = M("330.0000 UTR") - M("330.0000 UTR") * (bytes2 / 2) / bytes2;
   BOOST_REQUIRE_EQUAL( asset::from_string("10.0000 UTR").amount, total["net_weight"].as_uint64());
   BOOST_REQUIRE_EQUAL( asset::from_string("20.0000 UTR").amount, total["cpu_weight"].as_uint64());
...
BOOST_FIXTURE_TEST_CASE( adding_stake_partial_unstake, ultrainio_system_tester ) try {
   issue( "alice", "1000.0000 UTR",  config::system_account_name );
   BOOST_REQUIRE_EQUAL( success(), stake( "alice", "bob", "200.0000 UTR", "100.0000 UTR", "80.0000 UTR" ) );
   ...
   BOOST_REQUIRE_EQUAL( success(), stake( "alice", "bob", "100.0000 UTR", "50.0000 UTR", "40.0000 UTR" ) );

   total = get_total_stake( "bob" );
   BOOST_REQUIRE_EQUAL( asset::from_string("300.0000 UTR").amount, total["net_weight"].as_uint64());
   BOOST_REQUIRE_EQUAL( asset::from_string("150.0000 UTR").amount, total["cpu_weight"].as_uint64());
   BOOST_REQUIRE_EQUAL( asset::from_string("120.0000 UTR").amount, total["storage_stake"].as_uint64());
   auto bytes = total["storage_bytes"].as_uint64();
   BOOST_REQUIRE_EQUAL( true, bytes0 < bytes );
   REQUIRE_MATCHING_OBJECT( voter( "alice", "450.0000 UTR" ), get_voter_info( "alice" ) );
   BOOST_REQUIRE_EQUAL( asset::from_string("430.0000 UTR"), get_balance( "alice" ) );

   //unstake a share
   BOOST_REQUIRE_EQUAL( success(), unstake( "alice", "bob", "150.0000 UTR", "75.0000 UTR", bytes / 2 ) );

   total = get_total_stake( "bob" );
   BOOST_REQUIRE_EQUAL( asset::from_string("150.0000 UTR").amount, total["net_weight"].as_uint64());
   BOOST_REQUIRE_EQUAL( asset::from_string("75.0000 UTR").amount, total["cpu_weight"].as_uint64());
   BOOST_REQUIRE_EQUAL( bytes-bytes/2, total["storage_bytes"].as_uint64() );
   BOOST_REQUIRE_EQUAL( asset::from_string("120.0000 UTR").amount - asset::from_string("120.0000 UTR").amount * (bytes/2)/bytes,
                        total["storage_stake"].as_uint64());
   REQUIRE_MATCHING_OBJECT( voter( "alice", "225.0000 UTR" ), get_voter_info( "alice" ) );

   //unstake more
   BOOST_REQUIRE_EQUAL( success(), unstake( "alice", "bob", "50.0000 UTR", "25.0000 UTR", 0 ) );
   total = get_total_stake( "bob" );
   BOOST_REQUIRE_EQUAL( asset::from_string("100.0000 UTR").amount, total["net_weight"].as_uint64());
   BOOST_REQUIRE_EQUAL( asset::from_string("50.0000 UTR").amount, total["cpu_weight"].as_uint64());
   BOOST_REQUIRE_EQUAL( bytes-bytes/2, total["storage_bytes"].as_uint64() );
   REQUIRE_MATCHING_OBJECT( voter( "alice", "150.0000 UTR" ), get_voter_info( "alice" ) );
   ...
   produce_blocks(1);
   BOOST_REQUIRE_EQUAL( asset::from_string("790.0000 UTR"), get_balance( "alice" ) );
...
BOOST_FIXTURE_TEST_CASE( producer_register_unregister, ultrainio_system_tester ) try {
   ...
   vector<char> key = fc::raw::pack( fc::crypto::public_key( std::string("UTR6MRyAjQq8ud7hVNYcfnVPJqcVpscN5So8BhtHuGYqET5GDW5CV") ) );
   ...
   vector<char> key2 = fc::raw::pack( fc::crypto::public_key( std::string("UTRR16EPHFSKVYHBjQgxVGQPrwCxTg7BbZ69H9i4gztN9deKTEXYne4") ) );
   BOOST_REQUIRE_EQUAL( success(), push_action(N(alice), N(regproducer), mvo()
                                               ("producer",  "alice")
                                               ("producer_key", key2 )
                                               ("prefs", params2)
...
BOOST_FIXTURE_TEST_CASE( vote_for_producer, ultrainio_system_tester ) try {
   issue( "alice", "1000.0000 UTR",  config::system_account_name );
   fc::variant params = producer_parameters_example(1);
   ...
   issue( "bob", "2000.0000 UTR",  config::system_account_name );
   issue( "carol", "3000.0000 UTR",  config::system_account_name );

   //bob makes stake
   BOOST_REQUIRE_EQUAL( success(), stake( "bob", "11.0000 UTR", "0.1111 UTR", "0.0000 UTR" ) );
   BOOST_REQUIRE_EQUAL( asset::from_string("1988.8889 UTR"), get_balance( "bob" ) );
   REQUIRE_MATCHING_OBJECT( voter( "bob", "11.1111 UTR" ), get_voter_info( "bob" ) );
   ...
   //carol makes stake
   BOOST_REQUIRE_EQUAL( success(), stake( "carol", "22.0000 UTR", "0.2222 UTR", "0.0000 UTR" ) );
   REQUIRE_MATCHING_OBJECT( voter( "carol", "22.2222 UTR" ), get_voter_info( "carol" ) );
   BOOST_REQUIRE_EQUAL( asset::from_string("2977.7778 UTR"), get_balance( "carol" ) );
   ...
   BOOST_REQUIRE_EQUAL( success(), stake( "bob", "55.0000 UTR", "0.5555 UTR", "0.0000 UTR" ) );
   //should increase alice's total_votes
   prod = get_producer_info( "alice" );
   BOOST_REQUIRE_EQUAL( 888888, prod["total_votes"].as_uint64() );

   //carol unstakes part of the stake
   BOOST_REQUIRE_EQUAL( success(), unstake( "carol", "2.0000 UTR", "0.0002 UTR", 0 ) );
   ...
   //but ultrain should still be at stake
   BOOST_REQUIRE_EQUAL( asset::from_string("1933.3334 UTR"), get_balance( "bob" ) );

   //carol unstakes rest of ultrain
   BOOST_REQUIRE_EQUAL( success(), unstake( "carol", "20.0000 UTR", "0.2220 UTR", 0 ) );
   ...
   produce_block();
   BOOST_REQUIRE_EQUAL( asset::from_string("3000.0000 UTR"), get_balance( "carol" ) );

} FC_LOG_AND_RETHROW()


BOOST_FIXTURE_TEST_CASE( unregistered_producer_voting, ultrainio_system_tester ) try {
   issue( "bob", "2000.0000 UTR",  config::system_account_name );
   BOOST_REQUIRE_EQUAL( success(), stake( "bob", "13.0000 UTR", "0.5791 UTR", "0.0000 UTR" ) );
   REQUIRE_MATCHING_OBJECT( voter( "bob", "13.5791 UTR" ), get_voter_info( "bob" ) );
...
BOOST_FIXTURE_TEST_CASE( more_than_30_producer_voting, ultrainio_system_tester ) try {
   issue( "bob", "2000.0000 UTR",  config::system_account_name );
   BOOST_REQUIRE_EQUAL( success(), stake( "bob", "13.0000 UTR", "0.5791 UTR", "0.0000 UTR" ) );
   REQUIRE_MATCHING_OBJECT( voter( "bob", "13.5791 UTR" ), get_voter_info( "bob" ) );
...
BOOST_FIXTURE_TEST_CASE( vote_same_producer_30_times, ultrainio_system_tester ) try {
   issue( "bob", "2000.0000 UTR",  config::system_account_name );
   BOOST_REQUIRE_EQUAL( success(), stake( "bob", "50.0000 UTR", "50.0000 UTR", "0.0000 UTR" ) );
   REQUIRE_MATCHING_OBJECT( voter( "bob", "100.0000 UTR" ), get_voter_info( "bob" ) );
...
BOOST_FIXTURE_TEST_CASE( producer_keep_votes, ultrainio_system_tester ) try {
   issue( "alice", "1000.0000 UTR",  config::system_account_name );
   fc::variant params = producer_parameters_example(1);
   vector<char> key = fc::raw::pack( get_public_key( N(alice), "active" ) );
   BOOST_REQUIRE_EQUAL( success(), push_action( N(alice), N(regproducer), mvo()
...
BOOST_FIXTURE_TEST_CASE( vote_for_two_producers, ultrainio_system_tester ) try {
   //alice becomes a producer
   fc::variant params = producer_parameters_example(1);
   vector<char> key = fc::raw::pack( get_public_key( N(alice), "active" ) );
   BOOST_REQUIRE_EQUAL( success(), push_action( N(alice), N(regproducer), mvo()
   ...
   issue( "carol", "1000.0000 UTR",  config::system_account_name );
   BOOST_REQUIRE_EQUAL( success(), stake( "carol", "15.0005 UTR", "5.0000 UTR", "0.0000 UTR" ) );
   BOOST_REQUIRE_EQUAL( success(), push_action(N(carol), N(voteproducer), mvo()
                                               ("voter",  "carol")
                                               ("proxy", name(0).to_string() )
   ...
   issue( "alice", "2.0000 UTR",  config::system_account_name );
   BOOST_REQUIRE_EQUAL( success(), stake( "alice", "1.0000 UTR", "1.0000 UTR", "0.0000 UTR" ) );
...
BOOST_FIXTURE_TEST_CASE( proxy_register_unregister_keeps_stake, ultrainio_system_tester ) try {
   ...
   issue( "bob", "1000.0000 UTR",  config::system_account_name );
   BOOST_REQUIRE_EQUAL( success(), stake( "bob", "200.0002 UTR", "100.0001 UTR", "80.0000 UTR" ) );
   BOOST_REQUIRE_EQUAL( success(), push_action( N(bob), N(regproxy), mvo()
                                               ("proxy",  "bob")
                        )
   );
   REQUIRE_MATCHING_OBJECT( proxy( "bob" )( "staked", M("300.0003 UTR") ), get_voter_info( "bob" ) );
   //unrgister and check that stake is still in place
   BOOST_REQUIRE_EQUAL( success(), push_action( N(bob), N(unregproxy), mvo()
                                               ("proxy",  "bob")
                        )
   );
   REQUIRE_MATCHING_OBJECT( voter( "bob", "300.0003 UTR" ), get_voter_info( "bob" ) );
   ...
   issue( "carol", "1000.0000 UTR",  config::system_account_name );
   BOOST_REQUIRE_EQUAL( success(), stake( "carol", "246.0002 UTR", "531.0001 UTR", "80.0000 UTR" ) );
   //check that both proxy flag and stake a correct
   REQUIRE_MATCHING_OBJECT( proxy( "carol" )( "staked", M("777.0003 UTR") ), get_voter_info( "carol" ) );
   ...
   REQUIRE_MATCHING_OBJECT( voter( "carol", "777.0003 UTR" ), get_voter_info( "carol" ) );

} FC_LOG_AND_RETHROW()


BOOST_FIXTURE_TEST_CASE( proxy_stake_unstake_keeps_proxy_flag, ultrainio_system_tester ) try {
   BOOST_REQUIRE_EQUAL( success(), push_action( N(alice), N(regproxy), mvo()
                                               ("proxy",  "alice")
                        )
   );
   ...
   //stake
   BOOST_REQUIRE_EQUAL( success(), stake( "alice", "100.0000 UTR", "50.0000 UTR", "80.0000 UTR" ) );
   //check that account is still a proxy
   REQUIRE_MATCHING_OBJECT( proxy( "alice" )( "staked", M("150.0000 UTR") ), get_voter_info( "alice" ) );

   //stake more
   BOOST_REQUIRE_EQUAL( success(), stake( "alice", "30.0000 UTR", "20.0000 UTR", "80.0000 UTR" ) );
   //check that account is still a proxy
   REQUIRE_MATCHING_OBJECT( proxy( "alice" )("staked", M("200.0000 UTR") ), get_voter_info( "alice" ) );

   //unstake more
   BOOST_REQUIRE_EQUAL( success(), unstake( "alice", "65.0000 UTR", "35.0000 UTR", 0 ) );
   REQUIRE_MATCHING_OBJECT( proxy( "alice" )("staked", M("100.0000 UTR") ), get_voter_info( "alice" ) );

   //unstake the rest
   BOOST_REQUIRE_EQUAL( success(), unstake( "alice", "65.0000 UTR", "35.0000 UTR", 0 ) );
   REQUIRE_MATCHING_OBJECT( proxy( "alice" )( "staked", M("0.0000 UTR") ), get_voter_info( "alice" ) );

} FC_LOG_AND_RETHROW()


BOOST_FIXTURE_TEST_CASE( proxy_actions_affect_producers, ultrainio_system_tester ) try {
   ...
   issue( "bob", "1000.0000 UTR",  config::system_account_name );
   BOOST_REQUIRE_EQUAL( success(), stake( "bob", "100.0002 UTR", "50.0001 UTR", "50.0000 UTR" ) );
   BOOST_REQUIRE_EQUAL( success(), push_action(N(bob), N(voteproducer), mvo()
                                               ("voter",  "bob")
                                               ("proxy", "alice" )
                                               ("producers", vector<account_name>() )
   ...
   issue( "alice", "1000.0000 UTR",  config::system_account_name );
   BOOST_REQUIRE_EQUAL( success(), stake( "alice", "30.0001 UTR", "20.0001 UTR", "50.0000 UTR" ) );
   ...
   BOOST_REQUIRE_EQUAL( success(), unstake( "alice", "10.0001 UTR", "10.0001 UTR", 0 ) );
...
BOOST_FIXTURE_TEST_CASE(producer_pay, ultrainio_system_tester) try {
   issue( "alice", "10000000.0000 UTR",  config::system_account_name );
   ...
   BOOST_REQUIRE_EQUAL(success(), push_action(N(bob), N(delegatebw), mvo()
                                              ("from",     "bob")
                                              ("receiver", "bob")
                                              ("stake_net", "11.0000 UTR")
                                              ("stake_cpu", "00.1111 UTR")
                                              ("stake_storage", "0.0000 UTR")
                                              )
                       );
...
BOOST_FIXTURE_TEST_CASE( voters_actions_affect_proxy_and_producers, ultrainio_system_tester ) try {
   ...
   issue( "carol", "1000.0000 UTR",  config::system_account_name );
   BOOST_REQUIRE_EQUAL( success(), stake( "carol", "30.0001 UTR", "20.0001 UTR", "80.0000 UTR" ) );
   BOOST_REQUIRE_EQUAL( success(), push_action( N(carol), N(voteproducer), mvo()
                                                ("voter",  "carol")
                                                ("proxy", "alice" )
                                                ("producers", vector<account_name>() )
   ...
   BOOST_REQUIRE_EQUAL( success(), stake( "carol", "50.0000 UTR", "70.0000 UTR", "0.0000 UTR" ) );
   ...
   BOOST_REQUIRE_EQUAL( success(), unstake( "bob", "50.0001 UTR", "50.0001 UTR", 0 ) );
...
BOOST_FIXTURE_TEST_CASE( vote_both_proxy_and_producers, ultrainio_system_tester ) try {
...
BOOST_FIXTURE_TEST_CASE( select_invalid_proxy, ultrainio_system_tester ) try {
   //accumulate proxied votes
   issue( "bob", "1000.0000 UTR",  config::system_account_name );
   BOOST_REQUIRE_EQUAL( success(), stake( "bob", "100.0002 UTR", "50.0001 UTR", "50.0000 UTR" ) );
...
BOOST_FIXTURE_TEST_CASE( double_register_unregister_proxy_keeps_votes, ultrainio_system_tester ) try {
   //alice becomes a proxy
   BOOST_REQUIRE_EQUAL( success(), push_action( N(alice), N(regproxy), mvo()
                                                ("proxy",  "alice")
                        )
   ...
   issue( "alice", "1000.0000 UTR",  config::system_account_name );
   BOOST_REQUIRE_EQUAL( success(), stake( "alice", "5.0000 UTR", "5.0000 UTR", "50.0000 UTR" ) );
   REQUIRE_MATCHING_OBJECT( proxy( "alice" )( "staked", M("10.0000 UTR" ) ), get_voter_info( "alice" ) );
   ...
   REQUIRE_MATCHING_OBJECT( proxy( "alice" )( "proxied_votes", 1500003 )( "staked", M("10.0000 UTR" ) ), get_voter_info( "alice" ) );
   ...
   REQUIRE_MATCHING_OBJECT( voter( "alice" )( "proxied_votes", 1500003 )( "staked", M("10.0000 UTR" ) ), get_voter_info( "alice" ) );
...
BOOST_FIXTURE_TEST_CASE( proxy_cannot_use_another_proxy, ultrainio_system_tester ) try {
   //alice becomes a proxy
   BOOST_REQUIRE_EQUAL( success(), push_action( N(alice), N(regproxy), mvo()
                                                ("proxy",  "alice")
                        )
   ...
   issue( "carol", "1000.0000 UTR",  config::system_account_name );
   BOOST_REQUIRE_EQUAL( success(), stake( "carol", "100.0002 UTR", "50.0001 UTR", "50.0000 UTR" ) );
   BOOST_REQUIRE_EQUAL( success(), push_action( N(carol), N(voteproducer), mvo()
                                                ("voter",  "carol")
                                                ("proxy", "alice" )
                                                ("producers", vector<account_name>() )
...
fc::mutable_variant_object config_to_variant( const ultrainio::chain::chain_config& config ) {
   return mutable_variant_object()
...
BOOST_FIXTURE_TEST_CASE( elect_producers_and_parameters, ultrainio_system_tester ) try {
   create_accounts( {  N(producer1), N(producer2), N(producer3) } );
   BOOST_REQUIRE_EQUAL( success(), regproducer( "producer1", 1) );
   BOOST_REQUIRE_EQUAL( success(), regproducer( "producer2", 2) );
   BOOST_REQUIRE_EQUAL( success(), regproducer( "producer3", 3) );
   ...
   issue( "alice", "1000.0000 UTR",  config::system_account_name );
   BOOST_REQUIRE_EQUAL( success(), stake( "alice", "100.0000 UTR", "50.0000 UTR", "50.0000 UTR" ) );
   //vote for producers
   BOOST_REQUIRE_EQUAL( success(), push_action(N(alice), N(voteproducer), mvo()
                                               ("voter",  "alice")
                                               ("proxy", name(0).to_string() )
   ...
   // elect 2 producers
   issue( "bob", "1000.0000 UTR",  config::system_account_name );
   BOOST_REQUIRE_EQUAL( success(), stake( "bob", "200.0000 UTR", "100.0000 UTR", "50.0000 UTR" ) );
   BOOST_REQUIRE_EQUAL( success(), push_action(N(bob), N(voteproducer), mvo()
                                               ("voter",  "bob")
                                               ("proxy", name(0).to_string() )
                                               ("producers", vector<account_name>{ N(producer2) } )
