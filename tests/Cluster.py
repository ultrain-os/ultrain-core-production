...
class Cluster(object):
    ...
    def __init__(self, walletd=False, localCluster=True, host="localhost", port=8888, walletHost="localhost", walletPort=9899, enableMongo=False
                 , mongoHost="localhost", mongoPort=27017, mongoDb="UTRtest", defproduceraPrvtKey=None, defproducerbPrvtKey=None, staging=False):
        """Cluster container.
        ...
        self.ultrainioAccount=self.defProducerAccounts["ultrainio"]= Account("ultrainio")
    ...
    def launch(self, pnodes=1, totalNodes=1, prodCount=1, topo="mesh", p2pPlugin="net", delay=1, onlyBios=False, dontKill=False
        ...
        if not Cluster.arePortsAvailable(set(range(self.port, self.port+totalNodes+1))):
            Utils.Print("ERROR: Another process is listening on nodultrain default port.")
            return False
        ...
        nodultrainArgs="--max-transaction-time 5000 --abi-serializer-max-time-ms 5000 --filter-on * --p2p-max-nodes-per-host %d" % (totalNodes)
        if not self.walletd:
            nodultrainArgs += " --plugin ultrainio::wallet_api_plugin"
        if self.enableMongo:
            nodultrainArgs += " --plugin ultrainio::mongo_db_plugin --mongodb-wipe --delete-all-blocks --mongodb-uri %s" % self.mongoUri
        if Utils.Debug:
            nodultrainArgs += " --contracts-console"
        ...
        self.ultrainioAccount=self.defProducerAccounts["ultrainio"]
