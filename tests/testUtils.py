...
class Utils:
    ...
    def iAmNotNoon():
        ...
        Utils.EosClientPath="programs/ultrainc/ultrainc"

        Utils.EosWalletName="ultrain-walletd"
        Utils.EosWalletPath="programs/ultrain-walletd/"+ Utils.EosWalletName

        Utils.EosServerName="ultraind"
        Utils.EosServerPath="programs/ultraind/"+ Utils.EosServerName
...
class Node(object):
    ...
    def __init__(self, host, port, pid=None, cmd=None, alive=None, enableMongo=False, mongoHost="localhost", mongoPort=27017, mongoDb="UTRtest"):
        self.host=host
        self.port=port
        self.pid=pid
        self.cmd=cmd
    ...
    def getTransId_nj(trans):
        """Retrieve transaction id from clultrain non-json output."""
    ...
    def createInitAccounts(self, producers):
        """Initializes accounts. Requires ultrainio account. Creates init accounts and funds them."""
        ultrainio = Account("ultrainio")
        # ultrainio = self.ultrainioAccount

        # publish system contract
        contractDir="contracts/ultrainio.system"
        wastFile="contracts/ultrainio.system/ultrainio.system.wast"
        abiFile="contracts/ultrainio.system/ultrainio.system.abi"
        Utils.Print("Publish ultrainio.system contract")
        trans=self.publishContract(ultrainio.name, contractDir, wastFile, abiFile, waitForTransBlock=True)
        if trans is None:
           Utils.errorExit("Failed to publish ultrainio.system.")

        Utils.Print("push issue action to ultrainio contract")
        contract=ultrainio.name
        action="issue"
        data="{\"to\":\"ultrainio\",\"quantity\":\"1000000000.0000 ULTRAIN\"}"
        ...
        # Utils.Print("Verify ultrainio issue, Expected: %d" % (expectedAmount))
        # actualAmount=self.getAccountBalance(ultrainio.name)
        ...
        for name, keys in producers.items():
            ...
            trans=self.createAccount(initx, ultrainio, 0)
            if trans is None:
                Utils.errorExit("Failed to create account %s" % (name))

            trans = self.transferFunds(ultrainio, initx, 10000000000, "init transfer")
            if trans is None:
                Utils.errorExit("Failed to transfer funds from %s to %s." % (ultrainio.name, name))
    ...
    def getEosCurrencyBalance(self, name):
        cmd="%s %s get currency balance ultrainio %s UTR" % (Utils.EosClientPath, self.endpointArgs, name)
        Utils.Debug and Utils.Print("cmd: %s" % (cmd))
        try:
            ...
            msg=ex.output.decode("utf-8")
            Utils.Print("ERROR: Exception during get ULTRAIN balance. %s" % (msg))
            return None
...
class WalletMgr(object):
    ...
    def __init__(self, walletd, nodultrainPort=8888, nodultrainHost="localhost", port=8899, host="localhost"):
        ...
        self.endpointArgs="--host %s --port %d" % (self.nodultrainHost, self.nodultrainPort)
        if self.walletd:
...
class Cluster(object):
    ...
    def __init__(self, walletd=False, localCluster=True, host="localhost", port=8888, walletHost="localhost", walletPort=8899, enableMongo=False, mongoHost="localhost", mongoPort=27017, mongoDb="UTRtest", initaPrvtKey=None, initbPrvtKey=None, staging=False):
        self.accounts={}
        self.nodes={}
    ...
    def launch(self, pnodes=1, total_nodes=1, topo="mesh", delay=0):
        ...
        if not self.walletd or self.enableMongo:
            if Utils.amINoon:
                ...
                cmdArr.append("--ultraind")
            if not self.walletd:
                cmdArr.append("--plugin ultrainio::wallet_api_plugin")
            if self.enableMongo:
                if Utils.amINoon:
                    cmdArr.append("--plugin ultrainio::mongo_db_plugin --resync --mongodb-uri %s" % self.mongoUri)
                else:
                    cmdArr.append("--plugin ultrainio::db_plugin --mongodb-uri %s" % self.mongoUri)
        s=" ".join(cmdArr)
        Utils.Print("cmd: %s" % (s))
    ...
    def spreadFunds(self, amount=1):
        ...
        to=self.accounts[0]
        Utils.Print("Transfer %d units from account %s to %s on ultrain server port %d" % (
    ...
    def parseClusterInfo(self, totalNodes):
        ...
        configFile="etc/ultrainio/node_bios/config.ini"
        ...
        for i in range(0, totalNodes):
            configFile="etc/ultrainio/node_%02d/config.ini" % (i)
            Utils.Debug and Utils.Print("Parsing config file %s" % configFile)
        ...
        ultrainioKeys=producerKeys["ultrainio"]
        ...
        self.ultrainioAccount.ownerPrivateKey=ultrainioKeys[1]
        self.ultrainioAccount.ownerPublicKey=ultrainioKeys[0]
        self.ultrainioAccount.activePrivateKey=ultrainioKeys[1]
        self.ultrainioAccount.activePublicKey=ultrainioKeys[0]
    ...
    # Check state of running nodultrain process and update EosInstanceInfos
    #def updateEosInstanceInfos(ultrainInstanceInfos):
    ...
    def killall(self, silent=True):
        ...
        Utils.Debug and Utils.Print("cmd: %s" % (cmd))
        if 0 != subprocess.call(cmd.split(), stdout=Utils.FNull):
            not silent and Utils.Print("Launcher failed to shut down ultrain cluster.")

        # ocassionally the launcher cannot kill the ultrain server
        cmd="pkill %s" % (Utils.EosServerName)
        Utils.Debug and Utils.Print("cmd: %s" % (cmd))
        if 0 != subprocess.call(cmd.split(), stdout=Utils.FNull):
            not silent and Utils.Print("Failed to shut down ultrain cluster.")
