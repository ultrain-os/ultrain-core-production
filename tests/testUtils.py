...
class Utils:
    ...
    EosClientPath="programs/clultrain/clultrain"

    EosWalletName="kultraind"
    EosWalletPath="programs/kultraind/"+ EosWalletName

    EosServerName="nodultrain"
    EosServerPath="programs/nodultrain/"+ EosServerName

    EosLauncherPath="programs/ultrainio-launcher/ultrainio-launcher"
    MongoPath="mongo"
    ...
    # mongoSyncTime: nodultrain mongodb plugin seems to sync with a 10-15 seconds delay. This will inject
...
class Node(object):
    ...
    def __init__(self, host, port, pid=None, cmd=None, enableMongo=False, mongoHost="localhost", mongoPort=27017, mongoDb="UTRtest"):
        self.host=host
        self.port=port
        self.pid=pid
        self.cmd=cmd
    ...
    # Get actions mapped to an account (clultrain get actions)
    ...
    def getAccountEosBalanceStr(self, scope):
        """Returns SYS currency0000 account balance from clultrain get table command. Returned balance is string following syntax "98.0311 SYS". """
        assert isinstance(scope, str)
        if not self.enableMongo:
            amount=self.getTableAccountBalance("ultrainio.token", scope)
            ...
            if account is not None:
                balance=account["ultrain_balance"]
                return balance

        return None

    def getAccountEosBalance(self, scope):
        """Returns SYS currency0000 account balance from clultrain get table command. Returned balance is an integer e.g. 980311. """
    ...
    def getHeadBlockNum(self):
        """returns head block number(string) as returned by clultrain get info."""
        if not self.enableMongo:
            info=self.getInfo()
            if info is not None:
...
class WalletMgr(object):
    __walletLogFile="test_kultraind_output.log"
    __walletDataDir="test_wallet_0"

    # pylint: disable=too-many-arguments
    # walletd [True|False] True=Launch wallet(kultraind) process; False=Manage launch process externally.
    def __init__(self, walletd, nodultrainPort=8888, nodultrainHost="localhost", port=8899, host="localhost"):
        self.walletd=walletd
        self.nodultrainPort=nodultrainPort
        self.nodultrainHost=nodultrainHost
        self.port=port
        self.host=host
        self.wallets={}
        self.__walletPid=None
        self.endpointArgs="--url http://%s:%d" % (self.nodultrainHost, self.nodultrainPort)
    ...
    def launch(self):
        if not self.walletd:
            Utils.Print("ERROR: Wallet Manager wasn't configured to launch kultraind")
            return False
        ...
        # Give kultraind time to warm up
        time.sleep(1)
        return True
    ...
    def killall(self, allInstances=False):
        """Kill kultrain instances. allInstances will kill all kultrain instances running on the system."""
        if self.__walletPid:
...
class Cluster(object):
    ...
    # pylint: disable=too-many-arguments
    # walletd [True|False] Is kultraind running. If not load the wallet plugin
    def __init__(self, walletd=False, localCluster=True, host="localhost", port=8888, walletHost="localhost", walletPort=8899, enableMongo=False, mongoHost="localhost", mongoPort=27017, mongoDb="UTRtest", defproduceraPrvtKey=None, defproducerbPrvtKey=None, staging=False):
        """Cluster container.
        walletd [True|False] Is wallet kultraind running. If not load the wallet plugin
        localCluster [True|False] Is cluster local to host.
        host: ultrain server host
        port: ultrain server port
        walletHost: ultrain wallet host
        walletPort: wos wallet port
        enableMongo: Include mongoDb support, configures ultrain mongo plugin
        ...
        self.ultrainioAccount=Account("ultrainio")
    ...
    def launch(self, pnodes=1, totalNodes=1, prodCount=1, topo="mesh", delay=1, onlyBios=False, dontKill=False):
        ...
        nodultrainArgs="--max-transaction-time 5000 --filter-on *"
        if not self.walletd:
            nodultrainArgs += " --plugin ultrainio::wallet_api_plugin"
        if self.enableMongo:
            nodultrainArgs += " --plugin ultrainio::mongo_db_plugin --delete-all-blocks --mongodb-uri %s" % self.mongoUri

        if nodultrainArgs:
            cmdArr.append("--nodultrain")
            cmdArr.append(nodultrainArgs)
    ...
    def spreadFunds(self, source, accounts, amount=1):
        ...
        fromm=source
        to=accounts[0]
        Utils.Print("Transfer %s units from account %s to %s on ultrain server port %d" % (
    ...
    def validateSpreadFunds(self, initialBalances, transferAmount, source, accounts):
        ...
        for node in self.nodes:
            ...
            if node.validateFunds(initialBalances, transferAmount, source, accounts) is False:
                Utils.Print("ERROR: Failed to validate funds on ultrain node port: %d" % (node.port))
                return False
    ...
    def validateAccounts(self, accounts, testSysAccounts=True):
        ...
        myAccounts = []
        if testSysAccounts:
            myAccounts += [self.ultrainioAccount, self.defproduceraAccount, self.defproducerbAccount]
        if accounts:
            assert(isinstance(accounts, list))
            myAccounts += accounts
    ...
    def parseClusterKeys(totalNodes):
        ...
        node="node_bios"
        configFile="etc/ultrainio/%s/config.ini" % (node)
        if Utils.Debug: Utils.Print("Parsing config file %s" % configFile)
        producerKeys=Cluster.parseProducerKeys(configFile, node)
        if producerKeys is None:
            Utils.Print("ERROR: Failed to parse ultrainio private keys from cluster config files.")
            return None
    ...
    def bootstrap(totalNodes, prodCount, biosHost, biosPort, dontKill=False, onlyBios=False):
        ...
        try:
            ...
            ultrainioName="ultrainio"
            ultrainioKeys=producerKeys[ultrainioName]
            ultrainioAccount=Account(ultrainioName)
            ultrainioAccount.ownerPrivateKey=ultrainioKeys["private"]
            ultrainioAccount.ownerPublicKey=ultrainioKeys["public"]
            ultrainioAccount.activePrivateKey=ultrainioKeys["private"]
            ultrainioAccount.activePublicKey=ultrainioKeys["public"]

            if not walletMgr.importKey(ultrainioAccount, ignWallet):
                Utils.Print("ERROR: Failed to import %s account keys into ignition wallet." % (ultrainioName))
                return False

            contract="ultrainio.bios"
            contractDir="contracts/%s" % (contract)
            wastFile="contracts/%s/%s.wast" % (contract, contract)
            abiFile="contracts/%s/%s.abi" % (contract, contract)
            Utils.Print("Publish %s contract" % (contract))
            trans=biosNode.publishContract(ultrainioAccount.name, contractDir, wastFile, abiFile, waitForTransBlock=True)
            if trans is None:
            ...
            producerKeys.pop(ultrainioName)
            accounts=[]
            for name, keys in producerKeys.items():
                ...
                trans=biosNode.createAccount(initx, ultrainioAccount, 0)
                if trans is None:
            ...
            if not onlyBios:
                if prodCount == -1:
                    ...
                    with open(setProdsFile, "r") as f:
                        ...
                        opts="--permission ultrainio@active"
                        myTrans=biosNode.pushMessage("ultrainio", "setprods", setProdsStr, opts)
                    ...
                    trans=biosNode.pushMessage("ultrainio", "setprods", setProdsStr, opts)
                ...
                # wait for block production handover (essentially a block produced by anyone but ultrainio).
                lam = lambda: biosNode.getInfo()["head_block_producer"] != "ultrainio"
                ret=Utils.waitForBool(lam)
                if not ret:
                    Utils.Print("ERROR: Block production handover failed.")
                    return False

            ultrainioTokenAccount=copy.deepcopy(ultrainioAccount)
            ultrainioTokenAccount.name="ultrainio.token"
            trans=biosNode.createAccount(ultrainioTokenAccount, ultrainioAccount, 0)
            if trans is None:
                Utils.Print("ERROR: Failed to create account %s" % (ultrainioTokenAccount.name))
                return False

            ultrainioRamAccount=copy.deepcopy(ultrainioAccount)
            ultrainioRamAccount.name="ultrainio.ram"
            trans=biosNode.createAccount(ultrainioRamAccount, ultrainioAccount, 0)
            if trans is None:
                Utils.Print("ERROR: Failed to create account %s" % (ultrainioRamAccount.name))
                return False

            ultrainioRamfeeAccount=copy.deepcopy(ultrainioAccount)
            ultrainioRamfeeAccount.name="ultrainio.ramfee"
            trans=biosNode.createAccount(ultrainioRamfeeAccount, ultrainioAccount, 0)
            if trans is None:
                Utils.Print("ERROR: Failed to create account %s" % (ultrainioRamfeeAccount.name))
                return False

            ultrainioStakeAccount=copy.deepcopy(ultrainioAccount)
            ultrainioStakeAccount.name="ultrainio.stake"
            trans=biosNode.createAccount(ultrainioStakeAccount, ultrainioAccount, 0)
            if trans is None:
                Utils.Print("ERROR: Failed to create account %s" % (ultrainioStakeAccount.name))
                return False
            ...
            contract="ultrainio.token"
            contractDir="contracts/%s" % (contract)
            wastFile="contracts/%s/%s.wast" % (contract, contract)
            abiFile="contracts/%s/%s.abi" % (contract, contract)
            Utils.Print("Publish %s contract" % (contract))
            trans=biosNode.publishContract(ultrainioTokenAccount.name, contractDir, wastFile, abiFile, waitForTransBlock=True)
            if trans is None:
                Utils.Print("ERROR: Failed to publish contract %s." % (contract))
                return False

            # Create currency0000, followed by issue currency0000
            contract=ultrainioTokenAccount.name
            Utils.Print("push create action to %s contract" % (contract))
            action="create"
            data="{\"issuer\":\"%s\",\"maximum_supply\":\"1000000000.0000 %s\",\"can_freeze\":\"0\",\"can_recall\":\"0\",\"can_whitelist\":\"0\"}" % (ultrainioTokenAccount.name, CORE_SYMBOL)
            opts="--permission %s@active" % (contract)
            trans=biosNode.pushMessage(contract, action, data, opts)
            if trans is None or not trans[0]:
                Utils.Print("ERROR: Failed to push create action to ultrainio contract.")
                return False
            ...
            action="issue"
            data="{\"to\":\"%s\",\"quantity\":\"1000000000.0000 %s\",\"memo\":\"initial issue\"}" % (ultrainioAccount.name, CORE_SYMBOL)
            opts="--permission %s@active" % (contract)
            trans=biosNode.pushMessage(contract, action, data, opts)
            if trans is None or not trans[0]:
                Utils.Print("ERROR: Failed to push issue action to ultrainio contract.")
                return False
            ...
            Utils.Print("Verify ultrainio issue, Expected: %s" % (expectedAmount))
            actualAmount=biosNode.getAccountEosBalanceStr(ultrainioAccount.name)
            if expectedAmount != actualAmount:
                Utils.Print("ERROR: Issue verification failed. Excepted %s, actual: %s" %
                            (expectedAmount, actualAmount))
                return False

            contract="ultrainio.system"
            contractDir="contracts/%s" % (contract)
            wastFile="contracts/%s/%s.wast" % (contract, contract)
            abiFile="contracts/%s/%s.abi" % (contract, contract)
            Utils.Print("Publish %s contract" % (contract))
            ...
            action="transfer"
            for name, keys in producerKeys.items():
                data="{\"from\":\"%s\",\"to\":\"%s\",\"quantity\":\"%s\",\"memo\":\"%s\"}" % (ultrainioAccount.name, name, initialFunds, "init transfer")
                opts="--permission %s@active" % (ultrainioAccount.name)
                trans=biosNode.pushMessage(contract, action, data, opts)
                if trans is None or not trans[0]:
                    Utils.Print("ERROR: Failed to transfer funds from %s to %s." % (ultrainioTokenAccount.name, name))
                    return False
    ...
    # Kills a percentange of Eos instances starting from the tail and update ultrainInstanceInfos state
    ...
    def dumpErrorDetails(self):
        fileName="etc/ultrainio/node_bios/config.ini"
        Cluster.dumpErrorDetailImpl(fileName)
        fileName="var/lib/node_bios/stderr.txt"
        Cluster.dumpErrorDetailImpl(fileName)

        for i in range(0, len(self.nodes)):
            fileName="etc/ultrainio/node_%02d/config.ini" % (i)
            Cluster.dumpErrorDetailImpl(fileName)
            fileName="var/lib/node_%02d/stderr.txt" % (i)
            Cluster.dumpErrorDetailImpl(fileName)

    def killall(self, silent=True, allInstances=False):
        """Kill cluster nodultrain instances. allInstances will kill all nodultrain instances running on the system."""
        cmd="%s -k 9" % (Utils.EosLauncherPath)
        if Utils.Debug: Utils.Print("cmd: %s" % (cmd))
        if 0 != subprocess.call(cmd.split(), stdout=Utils.FNull):
            if not silent: Utils.Print("Launcher failed to shut down ultrain cluster.")

        if allInstances:
            # ocassionally the launcher cannot kill the ultrain server
            cmd="pkill -9 %s" % (Utils.EosServerName)
            if Utils.Debug: Utils.Print("cmd: %s" % (cmd))
            if 0 != subprocess.call(cmd.split(), stdout=Utils.FNull):
                if not silent: Utils.Print("Failed to shut down ultrain cluster.")
    ...
    def cleanup(self):
        for f in glob.glob("var/lib/node_*"):
            shutil.rmtree(f)
        for f in glob.glob("etc/ultrainio/node_*"):
            shutil.rmtree(f)
