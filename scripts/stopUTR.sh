...
docker ps | grep $NAME-[1-7]$ | awk '{print $1}' | xargs -i docker exec -d {} bash -c "rm -rf /root/.local/share/ultrainio/wssultrain "
docker ps | grep $NAME-[1-7]$ | awk '{print $1}' | xargs -i docker exec -d {} bash -c "rm -rf /root/log/ "
...
docker ps | grep $NAME-7$ | awk '{print $1}' | xargs -i docker exec -d {} bash -c "killall  rand.sh"
