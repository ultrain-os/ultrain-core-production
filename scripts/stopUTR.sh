...
docker ps | grep yufeng-[1-7]$ | awk '{print $1}' | xargs -i docker exec {} bash -c "killall 'sleep' "
docker ps | grep yufeng-[1-7]$ | awk '{print $1}' | xargs -i docker exec {} bash -c "killall 'logrotate.sh' "
