...
const getChainName = async (url) => {
    ...
    try {
        ...
        logger.info(params);
        let res = await axios.post(url + "/v1/chain/get_table_records", params,{timeout: constant.apiTimeConstants.DEFAULT_SEED_API_TIME});
...
getCurrencyStats = async (httpEndPoint) => {
    try {
        ...
        const rs = await axios.post(httpEndPoint+ "/v1/chain/get_currency_stats",{"code":"utrio.token", "symbol":"UGAS"},{timeout: constant.apiTimeConstants.DEFAULT_SEED_API_TIME});
        return rs.data;
    } catch (e) {
