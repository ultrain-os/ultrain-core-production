...
class whitelist_blacklist_tester {
   public:
      ...
      void init() {
         ...
         chain.emplace(cfg);
         chain->create_accounts({N(ultrainio.token), N(alice), N(bob), N(charlie)});
         chain->set_code(N(ultrainio.token), ultrainio_token_wast);
         chain->set_abi(N(ultrainio.token), ultrainio_token_abi);
         chain->push_action( N(ultrainio.token), N(create), N(ultrainio.token), mvo()
              ( "issuer", "ultrainio.token" )
              ( "maximum_supply", "1000000.00 TOK" )
         );
         chain->push_action( N(ultrainio.token), N(issue), N(ultrainio.token), mvo()
              ( "to", "ultrainio.token" )
      ...
      transaction_trace_ptr transfer( account_name from, account_name to, string quantity = "1.00 TOK" ) {
         return chain->push_action( N(ultrainio.token), N(transfer), from, mvo()
            ( "from", from )
            ( "to", to )
            ( "quantity", quantity )
            ( "memo", "" )
...
BOOST_AUTO_TEST_CASE( actor_whitelist ) { try {
   ...
   test.actor_whitelist = {N(ultrainio), N(ultrainio.token), N(alice)};
   test.init();

   test.transfer( N(ultrainio.token), N(alice), "1000.00 TOK" );
...
BOOST_AUTO_TEST_CASE( contract_whitelist ) { try {
   whitelist_blacklist_tester test;
   test.contract_whitelist = {N(ultrainio), N(ultrainio.token), N(bob)};
   test.init();

   test.transfer( N(ultrainio.token), N(alice), "1000.00 TOK" );

   test.transfer( N(alice), N(ultrainio.token) );
