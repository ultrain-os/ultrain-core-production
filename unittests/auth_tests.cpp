...
BOOST_AUTO_TEST_CASE(link_auths) { try {
   ...
   // Link authority for ultrainio reqauth action with alice's spending key
   chain.link_authority("alice", "ultrainio", "spending",  "reqauth");
   ...
   BOOST_CHECK_THROW( chain.link_authority("alice", "ultrainio", "spending",  "reqauth"), action_validate_exception);

   // Unlink alice with ultrainio reqauth
   chain.unlink_authority("alice", "ultrainio", "reqauth");
   ...
   // Link authority for any ultrainio action with alice's scud key
   chain.link_authority("alice", "ultrainio", "scud");
...
BOOST_AUTO_TEST_CASE(link_then_update_auth) { try {
   ...
   chain.link_authority("alice", "ultrainio", "first",  "reqauth");
...
try {
   ...
   // Creating account with ultrainio. prefix with privileged account
   chain.create_account("ultrainio.test1");

   // Creating account with ultrainio. prefix with non-privileged account, should fail
   BOOST_CHECK_EXCEPTION(chain.create_account("ultrainio.test2", "joe"), action_validate_exception,
                         fc_exception_message_is("only privileged accounts can have names that start with 'ultrainio.'"));
...
BOOST_AUTO_TEST_CASE( any_auth ) { try {
   ...
   chain.link_authority( "alice", "ultrainio", "ultrainio.any", "reqauth" );
   chain.link_authority( "bob", "ultrainio", "ultrainio.any", "reqauth" );

   /// this should succeed because ultrainio::reqauth is linked to any permission
   chain.push_reqauth("alice", { permission_level{N(alice), "spending"} }, { spending_priv_key });
...
try {
   ...
   using resource_usage_object = ultrainio::chain::resource_limits::resource_usage_object;
   using by_owner = ultrainio::chain::resource_limits::by_owner;
...
BOOST_AUTO_TEST_CASE( linkauth_special ) { try {
   ...
   auto validate_disallow = [&] (const char *type) {
      BOOST_REQUIRE_EXCEPTION(
         ...
         fc_exception_message_is(std::string("Cannot link ultrainio::") + std::string(type) + std::string(" to a minimum permission"))
      );
   };
