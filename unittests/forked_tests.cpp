...
BOOST_AUTO_TEST_CASE( forking ) try {
   ...
   auto r2 = c.create_accounts( {N(ultrainio.token)} );
   wdump((fc::json::to_pretty_string(r2)));
   c.set_code( N(ultrainio.token), ultrainio_token_wast );
   c.set_abi( N(ultrainio.token), ultrainio_token_abi );
   c.produce_blocks(10);


   auto cr = c.push_action( N(ultrainio.token), N(create), N(ultrainio.token), mutable_variant_object()
              ("issuer",       "ultrainio" )
   ...
   cr = c.push_action( N(ultrainio.token), N(issue), N(ultrainio), mutable_variant_object()
              ("to",       "dan" )
