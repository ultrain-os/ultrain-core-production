...
namespace ultrainio_system {

class ultrainio_system_tester : public TESTER {
public:

   ultrainio_system_tester()
   : ultrainio_system_tester([](TESTER& ) {}){}

   template<typename Lambda>
   ultrainio_system_tester(Lambda setup) {
      setup(*this);
   ...
   action_result buyram( const account_name& payer, account_name receiver, const asset& ultrainin ) {
      return push_action( payer, N(buyram), mvo()( "payer",payer)("receiver",receiver)("quant",ultrainin) );
   }
   ...
   */
   ...
   vector<name> active_and_vote_producers() {
      ...
      transfer( "ultrainio", "alice1111111", core_from_string("650000000.0000"), "ultrainio" );
      ...
      auto trace_auth = TESTER::push_action(config::system_account_name, updateauth::get_name(), config::system_account_name, mvo()
                                            ...
                                            ("auth",  authority(1, {key_weight{get_public_key( config::system_account_name, "active" ), 1}}, {
                                                  permission_level_weight{{config::system_account_name, config::ultrainio_code_name}, 1},
...
inline uint64_t M( const string& ultrain_str ) {
   return core_from_string( ultrain_str ).get_amount();
}
