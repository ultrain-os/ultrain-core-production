...
#include "ultrainio_system_tester.hpp"

using namespace ultrainio_system;

BOOST_AUTO_TEST_SUITE(ultrainio_system_tests)

BOOST_FIXTURE_TEST_CASE( buysell, ultrainio_system_tester ) try {
   ...
   BOOST_REQUIRE_EQUAL( success(), stake( "ultrainio", "alice1111111", core_from_string("200.0000"), core_from_string("100.0000") ) );
...
BOOST_FIXTURE_TEST_CASE( stake_unstake_with_transfer, ultrainio_system_tester ) try {
   cross_15_percent_threshold();
...
BOOST_FIXTURE_TEST_CASE( fail_without_auth, ultrainio_system_tester ) try {
   ...
   BOOST_REQUIRE_EQUAL( success(), stake( "ultrainio", "alice1111111", core_from_string("2000.0000"), core_from_string("1000.0000") ) );
...
BOOST_FIXTURE_TEST_CASE( stake_from_refund, ultrainio_system_tester ) try {
   cross_15_percent_threshold();
...
BOOST_FIXTURE_TEST_CASE( vote_for_producer, ultrainio_system_tester, * boost::unit_test::tolerance(1e+5) ) try {
   cross_15_percent_threshold();
   ...
   BOOST_REQUIRE_EQUAL( success(), unstake( "carol1111111", core_from_string("2.0000"), core_from_string("0.0002")/*"2.0000 UTR", "0.0002 UTR"*/ ) );
   ...
   //carol1111111 unstakes rest of ultrain
...
BOOST_FIXTURE_TEST_CASE( unregistered_producer_voting, ultrainio_system_tester, * boost::unit_test::tolerance(1e+5) ) try {
   issue( "bob111111111", core_from_string("2000.0000"),  config::system_account_name );
...
BOOST_FIXTURE_TEST_CASE( producer_keep_votes, ultrainio_system_tester, * boost::unit_test::tolerance(1e+5) ) try {
   issue( "alice1111111", core_from_string("1000.0000"),  config::system_account_name );
   fc::variant params = producer_parameters_example(1);
...
BOOST_FIXTURE_TEST_CASE( vote_for_two_producers, ultrainio_system_tester, * boost::unit_test::tolerance(1e+5) ) try {
   //alice1111111 becomes a producer
   fc::variant params = producer_parameters_example(1);
...
BOOST_FIXTURE_TEST_CASE( proxy_actions_affect_producers, ultrainio_system_tester, * boost::unit_test::tolerance(1e+5) ) try {
   cross_15_percent_threshold();
...
BOOST_FIXTURE_TEST_CASE(producer_pay, ultrainio_system_tester, * boost::unit_test::tolerance(1e-10)) try {
...
BOOST_FIXTURE_TEST_CASE(multiple_producer_pay, ultrainio_system_tester, * boost::unit_test::tolerance(1e-10)) try {
...

   {
      ...
      BOOST_REQUIRE_EQUAL( error("missing authority of ultrainio"),
...
BOOST_FIXTURE_TEST_CASE(producers_upgrade_system_contract, ultrainio_system_tester) try {
   ...
   string ultrainio_system_wast2 = ultrainio_system_wast;
   string msg = "producer votes must be unique and sorted";
   auto pos = ultrainio_system_wast2.find(msg);
   BOOST_REQUIRE( pos != std::string::npos );
   msg[0] = 'P';
   ultrainio_system_wast2.replace( pos, msg.size(), msg );

   transaction trx;
   {
      auto code = wast_to_wasm( ultrainio_system_wast2 );
...
BOOST_FIXTURE_TEST_CASE(producer_onblock_check, ultrainio_system_tester) try {
...
BOOST_FIXTURE_TEST_CASE( voters_actions_affect_proxy_and_producers, ultrainio_system_tester, * boost::unit_test::tolerance(1e+6) ) try {
   cross_15_percent_threshold();
...
BOOST_FIXTURE_TEST_CASE( elect_producers /*_and_parameters*/, ultrainio_system_tester ) try {
   create_accounts_with_resources( {  N(defproducer1), N(defproducer2), N(defproducer3) } );
   BOOST_REQUIRE_EQUAL( success(), regproducer( "defproducer1", 1) );
   BOOST_REQUIRE_EQUAL( success(), regproducer( "defproducer2", 2) );
   BOOST_REQUIRE_EQUAL( success(), regproducer( "defproducer3", 3) );
   ...
   transfer( "ultrainio", "alice1111111", core_from_string("600000000.0000"), "ultrainio" );
...
BOOST_FIXTURE_TEST_CASE( buyname, ultrainio_system_tester ) try {
   ...
   BOOST_REQUIRE_EXCEPTION( create_accounts_with_resources( { N(fail) }, N(dan) ), // dan shouldn't be able to create fail
                            ultrainio_assert_message_exception, ultrainio_assert_message_is( "no active bid for name" ) );
   ...
   BOOST_REQUIRE_EXCEPTION( create_accounts_with_resources( { N(nofail) }, N(dan) ), // dan shoudn't be able to do this, sam won
                            ultrainio_assert_message_exception, ultrainio_assert_message_is( "only highest bidder can claim" ) );
   ...
   transfer( "ultrainio", "nofail", core_from_string( "1000.0000" ) );
...
BOOST_FIXTURE_TEST_CASE( vote_producers_in_and_out, ultrainio_system_tester ) try {
...
BOOST_FIXTURE_TEST_CASE( setparams, ultrainio_system_tester ) try {
   //install multisig contract
   abi_serializer msig_abi_ser = initialize_multisig();
   auto producer_names = active_and_vote_producers();
   ...
   ultrainio::chain::chain_config params;
...
BOOST_FIXTURE_TEST_CASE( setram_effect, ultrainio_system_tester ) try {
