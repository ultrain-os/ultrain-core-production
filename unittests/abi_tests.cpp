...
auto get_resolver(const abi_def& abi = abi_def())
{
   return [&abi](const account_name &name) -> optional<abi_serializer> {
      return abi_serializer(ultrainio_contract_abi(abi));
   };
}
...
{ try {
   ...
   abi_serializer abis(ultrainio_contract_abi(abi));
   abis.validate();
...
{ try {
   ...
   abi_serializer abis(ultrainio_contract_abi(abi_def()));
...
{ try {
   ...
   abi_serializer abis(ultrainio_contract_abi(v.as<abi_def>()));
