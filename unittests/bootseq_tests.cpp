...
public:
   ...
   fc::variant get_global_state() {
    ...
    auto buyram( name payer, name receiver, asset ram ) {
       auto r = base_tester::push_action(N(ultrainio), N(buyram), payer, mvo()
                    ("payer", payer)
                    ("receiver", receiver)
                    ("quant", ram)
                    );
    ...
    auto delegate_bandwidth( name from, name receiver, asset net, asset cpu, uint8_t transfer = 1) {
       auto r = base_tester::push_action(N(ultrainio), N(delegatebw), from, mvo()
                    ("from", from )
                    ("receiver", receiver)
    ...
    auto register_producer(name producer) {
       auto r = base_tester::push_action(N(ultrainio), N(regproducer), producer, mvo()
    ...
    auto undelegate_bandwidth( name from, name receiver, asset net, asset cpu ) {
       auto r = base_tester::push_action(N(ultrainio), N(undelegatebw), from, mvo()
                    ("from", from )
                    ("receiver", receiver)
...
BOOST_FIXTURE_TEST_CASE( bootseq_test, bootseq_tester ) {
    try {
        ...
        for( const auto& a : test_genesis ) {
           create_account( a.aname, N(ultrainio) );
        }

        // Set ultrainio.system to ultrainio
        set_code_abi(N(ultrainio), ultrainio_system_wast, ultrainio_system_abi);
        ...
        auto votepro = [&]( account_name voter, vector<account_name> producers ) {
          ...
          base_tester::push_action(N(ultrainio), N(voteproducer), voter, mvo()
                                ("voter",  name(voter))
                                ("proxy", name(0) )
                                ("producers", producers)
                     );
        ...
        BOOST_TEST(active_schedule.producers.front().producer_name == "ultrainio");
        ...
        BOOST_REQUIRE_THROW(claim_rewards(N(runnerup1)), ultrainio_assert_message_exception);
        ...
        BOOST_REQUIRE_THROW(undelegate_bandwidth(N(b1), N(b1), core_from_string("49999500.0000"), core_from_string("49999500.0000")), ultrainio_assert_message_exception);
