...
public:
   ...
   transaction_trace_ptr create_account_with_resources( account_name a, account_name creator, asset ramfunds, bool multisig,
      ...
      trx.actions.emplace_back( get_action( N(ultrainio), N(buyram), vector<permission_level>{{creator,config::active_name}},
                                            mvo()
                                            ("payer", creator)
                                            ("receiver", a)
                                            ("quant", ramfunds) )
      ...
      trx.actions.emplace_back( get_action( N(ultrainio), N(delegatebw), vector<permission_level>{{creator,config::active_name}},
                                            mvo()
                                            ("from", creator)
                                            ("receiver", a)
...
BOOST_FIXTURE_TEST_CASE( update_system_contract_all_approve, ultrainio_msig_tester ) try {

   // required to set up the link between (ultrainio active) and (ultrainio.prods active)
   //
   //                  ultrainio active
   //                       |
   //             ultrainio.prods active (2/3 threshold)
   //             /         |        \             <--- implicitly updated in onblock action
   // alice active     bob active   carol active

   set_authority(N(ultrainio), "active", authority(1,
      vector<key_weight>{{get_private_key("ultrainio", "active").get_public_key(), 1}},
      vector<permission_level_weight>{{{N(ultrainio.prods), config::active_name}, 1}}), "owner",
      { { N(ultrainio), "active" } }, { get_private_key( N(ultrainio), "active" ) });
   ...
   create_account_with_resources( N(alice1111111), N(ultrainio), core_from_string("1.0000"), false );
   create_account_with_resources( N(bob111111111), N(ultrainio), core_from_string("0.4500"), false );
   ...
   vector<permission_level> action_perm = {{N(ultrainio), config::active_name}};
   ...
   // execute by alice to replace the ultrainio system contract
   transaction_trace_ptr trace;
   control->applied_transaction.connect([&]( const transaction_trace_ptr& t) { if (t->scheduled) { trace = t; } } );
   ...
   BOOST_REQUIRE_EXCEPTION( create_account_with_resources( N(alice1111112), N(ultrainio), core_from_string("1.0000"), false ),
                            ultrainio_assert_message_exception, ultrainio_assert_message_is("Unknown Test")

   );
} FC_LOG_AND_RETHROW()

BOOST_FIXTURE_TEST_CASE( update_system_contract_major_approve, ultrainio_msig_tester ) try {

   // set up the link between (ultrainio active) and (ultrainio.prods active)
   set_authority(N(ultrainio), "active", authority(1,
      vector<key_weight>{{get_private_key("ultrainio", "active").get_public_key(), 1}},
      vector<permission_level_weight>{{{N(ultrainio.prods), config::active_name}, 1}}), "owner",
      { { N(ultrainio), "active" } }, { get_private_key( N(ultrainio), "active" ) });
   ...
   BOOST_REQUIRE_EXCEPTION(
      ...
      ultrainio_assert_message_exception, ultrainio_assert_message_is("transaction authorization failed")
   );
