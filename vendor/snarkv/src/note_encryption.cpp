...
NoteEncryption<MLEN>::NoteEncryption(zero::uint256 hSig) : nonce(0), hSig(hSig) {
...
template<size_t MLEN>
NoteDecryption<MLEN>::NoteDecryption(zero::uint256 sk_enc) : sk_enc(sk_enc) {
                                         ...
                                         ) const
{
    ...
    // Regenerate keypair
    zero::uint256 epk = NoteEncryption<MLEN>::generate_pubkey(esk);
...
template<size_t MLEN>
zero::uint256 NoteEncryption<MLEN>::generate_privkey(const uint252 &a_sk)
{
    zero::uint256 sk = PRF_addr_sk_enc(a_sk);
...
template<size_t MLEN>
zero::uint256 NoteEncryption<MLEN>::generate_pubkey(const zero::uint256 &sk_enc)
{
    zero::uint256 pk;
...
{
    zero::uint256 rand = random_uint256();
    (*rand.begin()) &= 0x0F;
