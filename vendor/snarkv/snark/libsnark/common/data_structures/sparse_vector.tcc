...
{
    out << v.domain_size_ << OUTPUT_NEWLINE;
...
std::istream& operator>>(std::istream& in, sparse_vector<T> &v)
{
    ...
    if (v.domain_size_ > MAX_VECTOR_SIZE)
    {
        throw std::range_error("sparse_vector domain size exceeds limits");
    }
    size_t s = v.domain_size_;
    v.indices.resize(s);
