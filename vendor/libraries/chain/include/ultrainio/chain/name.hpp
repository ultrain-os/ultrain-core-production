...
#define N(X) ultrainio::chain::string_to_name(#X)
...
namespace std {
   template<> struct hash<ultrainio::chain::name> : private hash<uint64_t> {
      typedef ultrainio::chain::name argument_type;
...
namespace fc {
  class variant;
  void to_variant(const ultrainio::chain::name& c, fc::variant& v);
  void from_variant(const fc::variant& v, ultrainio::chain::name& check);
} // fc


FC_REFLECT( ultrainio::chain::name, (value) )
