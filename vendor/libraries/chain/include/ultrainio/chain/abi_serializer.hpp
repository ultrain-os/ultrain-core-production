...
namespace impl {
   ...
   constexpr bool single_type_requires_abi_v() {
      return std::is_base_of<transaction, T>::value ||
             ...
             std::is_same<T, packed_generated_transaction>::value ||
   ...
   struct abi_from_variant {
                           ...
                           size_t recursion_depth, const fc::time_point& deadline, const fc::microseconds& max_serialization_time )
      {
         ...
         ULTRAIN_ASSERT( fc::time_point::now() < deadline, abi_serialization_deadline_exception, "serialization time limit ${t}us exceeded", ("t", max_serialization_time) );
         const variant_object& vo = v.get_object();
         ULTRAIN_ASSERT(vo.contains("account"), packed_transaction_type_exception, "Missing account");
         ULTRAIN_ASSERT(vo.contains("name"), packed_transaction_type_exception, "Missing name");
         ...
         ULTRAIN_ASSERT(valid_empty_data || !act.data.empty(), packed_transaction_type_exception,
                           ...
                           size_t recursion_depth, const fc::time_point& deadline, const fc::microseconds& max_serialization_time )
      {
         ...
         ULTRAIN_ASSERT( fc::time_point::now() < deadline, abi_serialization_deadline_exception, "serialization time limit ${t}us exceeded", ("t", max_serialization_time) );
         const variant_object& vo = v.get_object();
         ULTRAIN_ASSERT(vo.contains("signatures"), packed_transaction_type_exception, "Missing signatures");
         ULTRAIN_ASSERT(vo.contains("compression"), packed_transaction_type_exception, "Missing compression");
         ...
         if( vo.contains("packed_trx") && vo["packed_trx"].is_string() && !vo["packed_trx"].as_string().empty() ) {
            ...
            ULTRAIN_ASSERT(vo.contains("transaction"), packed_transaction_type_exception, "Missing transaction");
            transaction trx;
            vector<bytes> context_free_data;
