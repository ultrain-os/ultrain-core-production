...
namespace ultrainio { namespace chain {
   ...
   namespace webassembly { namespace common {
      ...
      try {
         ...
         if(validating && mod_name != "env")
            ULTRAIN_ASSERT( false, wasm_exception, "importing from module that is not 'env': ${module}.${export}", ("module",mod_name)("export",export_name) );
         ...
         ULTRAIN_ASSERT( false, wasm_exception, "${module}.${export} unresolveable", ("module",mod_name)("export",export_name) );
         return false;
   ...
   class wasm_interface {
      public:
         ...
         //validates code -- does a WASM validation pass and checks the wasm against ULTRAINIO specific constraints
