      ...
      class iterator_cache {
         public:
            ...
            const table_id_object& get_table( table_id_object::id_type i )const {
               ...
               ULTRAIN_ASSERT( itr != _table_cache.end(), table_not_in_cache, "an invariant was broken, table should be in cache" );
            ...
            const table_id_object* find_table_by_end_iterator( int ei )const {
               ULTRAIN_ASSERT( ei < -1, invalid_table_iterator, "not an end iterator" );
            ...
            const T& get( int iterator ) {
               ULTRAIN_ASSERT( iterator != -1, invalid_table_iterator, "invalid iterator" );
               ULTRAIN_ASSERT( iterator >= 0, table_operation_not_permitted, "dereference of end iterator" );
               ULTRAIN_ASSERT( iterator < _iterator_to_object.size(), invalid_table_iterator, "iterator out of range" );
               auto result = _iterator_to_object[iterator];
               ULTRAIN_ASSERT( result, table_operation_not_permitted, "dereference of deleted object" );
               return *result;
            }

            void remove( int iterator ) {
               ULTRAIN_ASSERT( iterator != -1, invalid_table_iterator, "invalid iterator" );
               ULTRAIN_ASSERT( iterator >= 0, table_operation_not_permitted, "cannot call remove on end iterators" );
               ULTRAIN_ASSERT( iterator < _iterator_to_object.size(), invalid_table_iterator, "iterator out of range" );
...
//               context.require_write_lock( table_obj.scope );
   ...
   private:
      ...
      int db_drop_secondary_index(const ultrainio::chain::table_id_object * t_id, bool is_exec_deltab_limit, uint64_t& calc_num, uint64_t& calc_delram_bytes);
...
//FC_REFLECT(ultrainio::chain::apply_context::apply_results, (applied_actions)(deferred_transaction_requests)(deferred_transactions_count))
