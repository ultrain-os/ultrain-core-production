...
namespace ultrainio { namespace chain { namespace webassembly { namespace binaryen {

using namespace fc;
using namespace wasm;
using namespace ultrainio::chain::webassembly::common;
...
struct interpreter_interface : ModuleInstance::ExternalInterface {
   ...
   Literal callImport(Import *import, LiteralList &args) override
   {
      ...
      ULTRAIN_ASSERT(fn_iter != import_lut.end(), wasm_execution_error, "unknown import ${m}:${n}", ("m", import->module.c_str())("n", import->module.c_str()));
   ...
   {
      ULTRAIN_ASSERT(index < table.size(), wasm_execution_error, "callIndirect: bad pointer");
      auto* func = instance.wasm.getFunctionOrNull(table[index]);
      ULTRAIN_ASSERT(func, wasm_execution_error, "callIndirect: uninitialized element");
      ULTRAIN_ASSERT(func->params.size() == arguments.size(), wasm_execution_error, "callIndirect: bad # of arguments");

      for (size_t i = 0; i < func->params.size(); i++) {
         ULTRAIN_ASSERT(func->params[i] == arguments[i].type, wasm_execution_error, "callIndirect: bad argument type");
      }
      ULTRAIN_ASSERT(func->result == result, wasm_execution_error, "callIndirect: bad result type");
   ...
   void assert_memory_is_accessible(uint32_t offset, uint32_t size) {
      ULTRAIN_ASSERT(offset + size <= current_memory_size && offset + size >= offset,
...
class binaryen_runtime : public ultrainio::chain::wasm_runtime_interface {
   public:
      binaryen_runtime();
...
inline auto convert_native_to_literal(const interpreter_interface* interface, char* ptr) {
   ...
   ULTRAIN_ASSERT(ptr >= base && ptr < top_of_memory, wasm_execution_error, "returning pointer not in linear memory");
...
#define _REGISTER_BINARYEN_INTRINSIC(CLS, MOD, METHOD, WASM_SIG, NAME, SIG)\
   static ultrainio::chain::webassembly::binaryen::intrinsic_registrator _INTRINSIC_NAME(__binaryen_intrinsic_fn, __COUNTER__) (\
      MOD "." NAME,\
      ultrainio::chain::webassembly::binaryen::intrinsic_function_invoker_wrapper<SIG>::type::fn<&CLS::METHOD>()\
   );\


} } } }// ultrainio::chain::webassembly::wavm
