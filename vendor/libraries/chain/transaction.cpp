...
void transaction_header::validate()const {
   ULTRAIN_ASSERT( max_net_usage_words.value < UINT32_MAX / 8UL, transaction_exception,
...
{ try {
   ...
   for(const signature_type& sig : signatures) {
      ...
      ULTRAIN_ASSERT( allow_duplicate_keys || successful_insertion, tx_duplicate_sig,
...
uint32_t packed_transaction::get_unprunable_size()const {
   ...
   ULTRAIN_ASSERT( size <= std::numeric_limits<uint32_t>::max(), tx_too_big, "packed_transaction is too big" );
...
struct read_limiter {
   ...
   {
      ULTRAIN_ASSERT(_total + count <= Limit, tx_decompression_error, "Exceeded maximum decompressed transaction size");
      _total += count;
...
bytes packed_transaction::get_raw_transaction() const
{
   try {
      switch(compression) {
         ...
         default:
            ULTRAIN_THROW(unknown_transaction_compression, "Unknown transaction compression algorithm");
      }
