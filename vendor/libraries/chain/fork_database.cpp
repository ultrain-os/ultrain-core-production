...
namespace ultrainio { namespace chain {
   ...
   void fork_database::set( block_state_ptr s ) {
      ...
      ULTRAIN_ASSERT( result.second, fork_database_exception, "unable to insert block state, duplicate state detected" );
      if( !my->head ) {
         my->head =  s;
   ...
   block_state_ptr fork_database::add( block_state_ptr n ) {
      ...
      ULTRAIN_ASSERT( inserted.second, fork_database_exception, "duplicate block added?" );
   ...
   block_state_ptr fork_database::add( signed_block_ptr b, bool trust ) {
      ULTRAIN_ASSERT( b, fork_database_exception, "attempt to add null block" );
      ULTRAIN_ASSERT( my->head, fork_db_block_not_found, "no head block set" );

      const auto& by_id_idx = my->index.get<by_block_id>();
      auto existing = by_id_idx.find( b->id() );
      ULTRAIN_ASSERT( existing == by_id_idx.end(), fork_database_exception, "we already know about this block" );

      auto prior = by_id_idx.find( b->previous );
      ULTRAIN_ASSERT( prior != by_id_idx.end(), unlinkable_block_exception, "unlinkable block", ("id", string(b->id()))("previous", string(b->previous)) );

      auto result = std::make_shared<block_state>( **prior, move(b), trust );
      ULTRAIN_ASSERT( result, fork_database_exception , "fail to add new block state" );
      return add(result);
   }
   ...
   pair< branch_type, branch_type >  fork_database::fetch_branch_from( const block_id_type& first,
      ...
      while( first_branch->block_num > second_branch->block_num )
      {
         ...
         ULTRAIN_ASSERT( first_branch, fork_db_block_not_found, "block ${id} does not exist", ("id", string(first_branch->header.previous)) );
      }
      ...
      while( second_branch->block_num > first_branch->block_num )
      {
         ...
         ULTRAIN_ASSERT( second_branch, fork_db_block_not_found, "block ${id} does not exist", ("id", string(second_branch->header.previous)) );
      }
   ...
   void fork_database::mark_in_current_chain( const block_state_ptr& h, bool in_current_chain ) {
      ...
      ULTRAIN_ASSERT( itr != by_id_idx.end(), fork_db_block_not_found, "could not find block in fork database" );
