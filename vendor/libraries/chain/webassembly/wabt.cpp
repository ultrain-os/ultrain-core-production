...
namespace wasm_constraints = ultrainio::chain::wasm_constraints;
...
class wabt_instantiated_module : public wasm_instantiated_module_interface {
   public:
      ...
      void apply(apply_context& context) override {
         ...
         _params[2].set_i64(uint64_t(context.act.name.valueH));
         _params[3].set_i64(uint64_t(context.act.name.valueL));

         ExecResult res = _executor.RunStartFunction(_instatiated_module);
         ULTRAIN_ASSERT( res.result == interp::Result::Ok, wasm_execution_error, "wabt start function failure (${s})", ("s", ResultToString(res.result)) );

         res = _executor.RunExportByName(_instatiated_module, "apply", _params);
         ULTRAIN_ASSERT( res.result == interp::Result::Ok, wasm_execution_error, "wabt execution failure (${s})", ("s", ResultToString(res.result)) );
      }
   ...
   private:
      ...
      TypedValues                                       _params{4, TypedValue(Type::I64)};
...
std::unique_ptr<wasm_instantiated_module_interface> wabt_runtime::instantiate_module(const char* code_bytes, size_t code_size, std::vector<uint8_t> initial_memory) {
   ...
   ULTRAIN_ASSERT( Succeeded(res), wasm_execution_error, "Error building wabt interp: ${e}", ("e", wabt::FormatErrorsToString(errors, Location::Type::Binary)) );
