...
*/
   ...
   // TODO: init http client here or in init() ?
   }
...
//         edump((s->block_num)(s->block->previous)(log_head->id()));
   ...
   void commit_block( bool add_to_fork_db ) {
      if( add_to_fork_db ) {
         ...
         //ilog("emit accepted_block_header block_num = ${block_num}", ("block_num", pending->_pending_block_state->block_num));
  ...
  //    ilog((fc::json::to_pretty_string(*pending->_pending_block_state->block)));
   ...
   void apply_block( const signed_block_ptr& b, controller::block_status s ) { try {
	 ...
	 hp->proposerProof = b->proposerProof;
         ...
         finalize_block();
	 FC_ASSERT( b->id() == pending->_pending_block_state->header.id(),
