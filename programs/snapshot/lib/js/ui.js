...
const update_title_bar_progress = (stage = "") => {
  ...
  switch( stage ){
    ...
    case 'distribute':
      title = `ULTRAIN | Snapshot ${debug.rates.percent_complete}% | ${debug.registry.accepted+debug.registry.rejected}/${debug.registry.total}`
    break
    case 'registry':
      title = `ULTRAIN | Building Registry | ${registrants.length}`
    break
    case 'reclaimer':
      title = `ULTRAIN | Reclaimable TXs | ${ debug.reclaimable.total }`
    break
    case 'complete':
      title = `ULTRAIN | Snapshot Complete`
    break
    default:
      title = `ULTRAIN | Welcome`
    break
  }
...
ui_refresh = setInterval( () => {
  ...
  if( disconnected ) {
    ...
    document.getElementsByClassName('disconnected')[0].innerHTML = `
      ...
      See <a href="https://github.com/ULTRAINIO/ultrain/blob/master/programs/snapshot/readme.md">Readme</a> for more information
      </p>
