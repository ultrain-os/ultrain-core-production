...
- `cache` (true, false) Will cache successful fallbacks, including full paper trail: Block number, address, transaction, public key and generated ULTRAIN key. Can speed up the slow fallback method if running several times.
...
1. ULTRAIN Balance of an address must be greater-than or equal to one ULTRAIN to be considered for inclusion in Genesis block
2. An ULTRAIN key must be associated to an account either by registration or fallback
...
3. If an ethereum address sent ULTRAIN ERC20 tokens to the ULTRAIN Crowdsale or ULTRAIN Token contract within the block range of the snapshot, these tokens are allocated to the respective address. 
4. If an address has unclaimed ULTRAIN in the ULTRAIN Crowdsale contract within block range, these tokens are allocated to the respective address.
...
2. Compile list of every address that has ever had an ULTRAIN balance, for each address:
	...
	3. Validate
		...
		2. Validate ULTRAIN Key, if valid set `registered` to `true`
		3. If ULTRAIN key error, save error to column `register_error`
	...
	6. Fallback _if `balance is gte 1` and `register_error is not null`_
	   1. Fast Fallback
		   1. Check database for any outgoing transactions (or keys cache), if found...
	   			...
	   			2. Generate and validate ULTRAIN Key
	   			3. If found and valid, set `valid` to `true` otherwise save error into `fallback_error` column
		2. Slow fallback
			1. Obtain list of unique addresses with balance gte 1 ULTRAIN and no ULTRAIN key and save them as keys in index (redis is used as index)
    ...
    6. Test
    	1. Daily Totals from DB against daily totals from ULTRAIN Utility Contract, failure here would not fail the below tests, but would instead result in inaccurate unclaimed balances. Difficult problem to detect without this test. 
    ...
    7. Output
    	1. **snapshot.csv** - comma-delimited list of ETH addresses, ULTRAIN keys and Total Balances (user, key, balance respectively)
...
With the output, you can use the Genesis Generator to create a well-formed genesis.json file for the ULTRAIN platform. 
...
- Testnet snapshots will produce accurate output based on period by constraining all blockchain activity to that range. This means that some seemingly superfluous calculations are conducted for testnet
	...
	- ULTRAIN Key Registration is concluded by last registration within the block range. 
- Mainnet simplifies a few things. However, it would be recommended that a cutoff block be enforced to encourage network consensus (primarily for registration transactions)
	...
	- ULTRAIN key registration is inferred from state returned by `keys` map in Crowdsale Contract. 
