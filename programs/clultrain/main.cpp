...
Options:
  ...
  -H,--host TEXT=localhost    the host where nodultrain is running
  -p,--port UINT=8888         the port where nodultrain is running
  --wallet-host TEXT=localhost
                              the host where kultraind is running
  --wallet-port UINT=8888     the port where kultraind is running
...
#include <ultrainio/chain/transaction_trace.hpp>
...
fc::variant call( const std::string& server, uint16_t port,
                  ...
                  const T& v ) { return ultrainio::client::http::call( server, port, path, fc::variant(v) ); }

template<typename T>
fc::variant call( const std::string& path,
                  const T& v ) { return ultrainio::client::http::call( host, port, path, fc::variant(v) ); }

ultrainio::chain_apis::read_only::get_info_results get_info() {
  return call(host, port, get_info_func ).as<ultrainio::chain_apis::read_only::get_info_results>();
}
...
void send_transaction( signed_transaction& trx, int32_t extra_kcpu, packed_transaction::compression_type compression = packed_transaction::none  ) {
   ...
   if( tx_print_json ) {
      ...
      auto trace = result["processed"].as<ultrainio::chain::transaction_trace>();
      print_result( result );
   }
}
...
chain::action create_newaccount(const name& creator, const name& newaccount, public_key_type owner, public_key_type active) {
   return action {
      ...
      contracts::newaccount{
         ...
         .recovery     = ultrainio::chain::authority{1, {}, {{{creator, config::active_name}, 1}}}
      }
   };
}
...
int main( int argc, char** argv ) {
   ...
   app.add_option( "-H,--host", host, localized("the host where nodultrain is running"), true );
   app.add_option( "-p,--port", port, localized("the port where nodultrain is running"), true );
   app.add_option( "--wallet-host", wallet_host, localized("the host where kultraind is running"), true );
   app.add_option( "--wallet-port", wallet_port, localized("the port where kultraind is running"), true );
   ...
   try {
      ...
      if (errorString.find("Connection refused") != string::npos) {
         if (errorString.find(fc::json::to_string(port)) != string::npos) {
            std::cerr << localized("Failed to connect to nodultrain at ${ip}:${port}; is nodultrain running?", ("ip", host)("port", port)) << std::endl;
         } else if (errorString.find(fc::json::to_string(wallet_port)) != string::npos) {
            std::cerr << localized("Failed to connect to kultraind at ${ip}:${port}; is kultraind running?", ("ip", wallet_host)("port", wallet_port)) << std::endl;
         } else {
