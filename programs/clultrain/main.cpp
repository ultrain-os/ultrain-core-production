...
fc::variant call( const std::string& url,
                  ...
                  const T& v ) {
   try {
      ultrainio::client::http::connection_param *cp = new ultrainio::client::http::connection_param((std::string&)url, (std::string&)path,
              no_verify ? false : true, headers);

      return ultrainio::client::http::do_http_call( *cp, fc::variant(v) );
   }
...
authority parse_json_authority_or_key(const std::string& authorityJsonOrFile) {
   if (boost::istarts_with(authorityJsonOrFile, "UTR")) {
      try {
...
void ensure_kultraind_running(CLI::App* app) {
    ...
    if (port_used(uint16_t(wallet_port)))  // Hopefully taken by kultraind
        return;
   ...
   delegate_bandwidth_subcommand(CLI::App* actionRoot) {
      ...
      delegate_bandwidth->set_callback([this] {
         fc::variant act_payload = fc::mutable_variant_object()
                  ...
                  ;
                  wdump((act_payload));
         send_actions({create_action({permission_level{from_str,config::active_name}}, config::system_account_name, NEX(delegatebw), act_payload)});
      });
   }
};
...
struct buyram_subcommand {
   ...
   buyram_subcommand(CLI::App* actionRoot) {
      ...
      buyram->add_option("tokens", amount, localized("The amount of UTR to pay for RAM"))->required();
...
int main( int argc, char** argv ) {
   ...
   CLI::App app{"Command Line Interface to ULTRAINIO Client"};
...
//                                ->check(CLI::ExistingFile);
   ...
   propose_action->set_callback([&] {
      ...
      trx.delay_sec = 0;
      trx.actions = { chain::action(trxperm, name(proposed_contract), action_name(proposed_action), proposed_trx_serialized) };
