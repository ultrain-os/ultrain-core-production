...
Subcommands:
  ...
  transfer                    Transfer ULTRAIN from account to account
...
fc::variant call( const std::string& url,
                  ...
                  const T& v ) {
   try {
      ultrainio::client::http::connection_param *cp = new ultrainio::client::http::connection_param(context, parse_url(url) + path,
              no_verify ? false : true, headers);

      return ultrainio::client::http::do_http_call( *cp, fc::variant(v), print_request, print_response );
   }
   ...
   create_account_subcommand(CLI::App* actionRoot, bool s) : simple(s) {
      ...
      if (!simple) {
         createAccount->add_option("--stake-net", stake_net,
                                   (localized("The amount of UTR delegated for net bandwidth")))->required();
         createAccount->add_option("--stake-cpu", stake_cpu,
                                   (localized("The amount of UTR delegated for CPU bandwidth")))->required();
         ...
         createAccount->add_option("--buy-ram", buy_ram_ultrain,
                                   (localized("The amount of RAM bytes to purchase for the new account in UTR")));
         createAccount->add_flag("--transfer", transfer,
                                 (localized("Transfer voting power and right to unstake UTR to receiver")));
      }
...
struct delegate_bandwidth_subcommand {
   ...
   delegate_bandwidth_subcommand(CLI::App* actionRoot) {
      ...
      delegate_bandwidth->add_option("stake_net_quantity", stake_net_amount, localized("The amount of UTR to stake for network bandwidth"))->required();
      delegate_bandwidth->add_option("stake_cpu_quantity", stake_cpu_amount, localized("The amount of UTR to stake for CPU bandwidth"))->required();
      delegate_bandwidth->add_option("--buyram", buy_ram_amount, localized("The amount of UTR to buyram"));
      delegate_bandwidth->add_flag("--transfer", transfer, localized("Transfer voting power and right to unstake UTR to receiver"));
...
struct undelegate_bandwidth_subcommand {
   ...
   undelegate_bandwidth_subcommand(CLI::App* actionRoot) {
      ...
      undelegate_bandwidth->add_option("unstake_net_quantity", unstake_net_amount, localized("The amount of UTR to undelegate for network bandwidth"))->required();
      undelegate_bandwidth->add_option("unstake_cpu_quantity", unstake_cpu_amount, localized("The amount of UTR to undelegate for CPU bandwidth"))->required();
...
struct bidname_subcommand {
   ...
   bidname_subcommand(CLI::App *actionRoot) {
      ...
      bidname->add_option("bid", bid_amount, localized("The amount of UTR to bid"))->required();
      ...
      bidname->set_callback([this] {
         ...
         send_actions({create_action({permission_level{bidder_str, config::active_name}}, config::system_account_name, NEX(bidname), act_payload)});
      });
   }
};
   ...
   bidname_info_subcommand(CLI::App* actionRoot) {
      ...
      list_producers->set_callback([this] {
         auto rawResult = call(get_table_func, fc::mutable_variant_object("json", true)
                               ("code", "ultrainio")("scope", "ultrainio")("table", "namebids")
                               ("lower_bound", ultrainio::chain::string_to_name(newname_str.c_str()))("limit", 1));
         if ( print_json ) {
            std::cout << fc::json::to_pretty_string(rawResult) << std::endl;
            return;
         }
...
struct buyram_subcommand {
   ...
   buyram_subcommand(CLI::App* actionRoot) {
      ...
      buyram->add_option("amount", amount, localized("The amount of UTR to pay for RAM, or number of kbytes of RAM if --kbytes is set"))->required();
...
struct sellram_subcommand {
   ...
   sellram_subcommand(CLI::App* actionRoot) {
      ...
      sellram->add_option("account", receiver_str, localized("The account to receive UTR for sold RAM"))->required();
...
//                                ->check(CLI::ExistingFile);
   ...
   string amount;
   string memo;
   auto transfer = app.add_subcommand("transfer", localized("Transfer UTR from account to account"), false);
   transfer->add_option("sender", sender, localized("The account sending UTR"))->required();
   transfer->add_option("recipient", recipient, localized("The account receiving UTR"))->required();
   transfer->add_option("amount", amount, localized("The amount of UTR to send"))->required();
