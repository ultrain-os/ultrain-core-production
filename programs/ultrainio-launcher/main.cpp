...
public:
  ...
  ultraind_def*       instance;
  string          gelf_endpoint;
};

void
ultraind_def::mk_dot_label () {
  dot_label_str = name + "\\nprod=";
...
void
ultraind_def::set_host( host_def* h, bool is_bios ) {
  host = h->host_name;
...
struct server_name_def {
  ...
  string ultrainio_home;
  uint16_t instances;
  server_name_def () : ipaddr(), name(), has_bios(false), ultrainio_home(), instances(1) {}
};
...
struct launcher_def {
  ...
  producer_set_def producer_set;
   ...
   void assign_name (ultraind_def &node, bool is_bios);
  ...
  void prep_remote_config_dir (ultraind_def &node, host_def *host);
  void launch (ultraind_def &node, string &gts);
  void kill (launch_modes mode, string sig_opt);
  pair<host_def, ultraind_def> find_node(uint16_t node_num);
  vector<pair<host_def, ultraind_def>> get_nodes(const string& node_number_list);
...
launcher_def::set_options (bpo::options_description &cfg) {
  cfg.add_options()
    ...
    ("skip-signature", bpo::bool_switch(&skip_transaction_signatures)->default_value(false), "nodultrain does not require transaction signatures.")
    ("nodultrain", bpo::value<string>(&ultraind_extra_args), "forward nodultrain command line argument(s) to each instance of nodultrain, enclose arg in quotes")
    ...
    ("per-host",bpo::value<int>(&per_host)->default_value(0),"specifies how many nodultrain instances will run on a single host. Use 0 to indicate all on one.")
...
launcher_def::initialize (const variables_map &vmap) {
  ...
  if( !host_map_file.empty() ) {
    try {
      ...
      for (auto &binding : bindings) {
        for (auto &ultraind : binding.instances) {
          ultraind.host = binding.host_name;
          ultraind.p2p_endpoint = binding.public_name + ":" + boost::lexical_cast<string,uint16_t>(ultraind.p2p_port);

          aliases.push_back (ultraind.name);
        }
      }
  ...
  char* erd_env_var = getenv ("ULTRAINIO_HOME");
...
void
launcher_def::assign_name (ultraind_def &node, bool is_bios) {
...
launcher_def::define_network () {
  ...
  if (per_host == 0) {
    host_def local_host;
    local_host.ultrainio_home = erd;
    local_host.genesis = genesis.string();
    for (size_t i = 0; i < (total_nodes); i++) {
      ultraind_def ultraind;
      assign_name(ultraind, i == 0);
      aliases.push_back(ultraind.name);
      ultraind.set_host (&local_host, i == 0);
      local_host.instances.emplace_back(move(ultraind));
    }
    ...
    for (size_t i = total_nodes; i > 0; i--) {
      ...
      if (ph_count == 0) {
        ...
        lhost->ultrainio_home =
          (local_id.contains (lhost->host_name) || servers.default_ultrainio_home.empty()) ?
          erd : servers.default_ultrainio_home;
        host_ndx++;
      } // ph_count == 0

      ultraind_def ultraind;
      assign_name(ultraind, do_bios);
      ultraind.has_db = false;

      if (servers.db.size()) {
        for (auto &dbn : servers.db) {
          if (lhost->host_name == dbn) {
            ultraind.has_db = true;
            break;
         }
        }
      }
      aliases.push_back(ultraind.name);
      ultraind.set_host (lhost, do_bios);
      do_bios = false;
      lhost->instances.emplace_back(move(ultraind));
      --ph_count;
    } // for i
    bindings.emplace_back( move(*lhost) );
    delete lhost;
...
launcher_def::deploy_config_files (tn_node_def &node) {
  boost::system::error_code ec;
  ultraind_def &instance = *node.instance;
  ...
  if (host->is_local()) {
    bfs::path cfgdir = bfs::path(host->ultrainio_home) / instance.config_dir_name;
    bfs::path dd = bfs::path(host->ultrainio_home) / instance.data_dir_name;
    ...
    bfs::path rfile = bfs::path (host->ultrainio_home) / instance.config_dir_name / "config.ini";
    ...
    rfile = bfs::path (host->ultrainio_home) / instance.config_dir_name / "logging.json";
    ...
    rfile = bfs::path (host->ultrainio_home) / instance.config_dir_name / "genesis.json";
...
launcher_def::write_config_file (tn_node_def &node) {
  ...
  if ( p2p == p2p_plugin::NET ) {
    cfg << "plugin = ultrainio::net_plugin\n";
  } else {
    cfg << "plugin = ultrainio::bnet_plugin\n";
  }
  cfg << "plugin = ultrainio::chain_api_plugin\n"
...
launcher_def::init_genesis () {
  bfs::path genesis_path = bfs::current_path() / "genesis.json";
   ...
   if (!src.good()) {
      ...
      ultrainio::chain::genesis_state default_genesis;
...
void
launcher_def::prep_remote_config_dir (ultraind_def &node, host_def *host) {
  bfs::path abs_config_dir = bfs::path(host->ultrainio_home) / node.config_dir_name;
  bfs::path abs_data_dir = bfs::path(host->ultrainio_home) / node.data_dir_name;

  string acd = abs_config_dir.string();
  string add = abs_data_dir.string();
  string cmd = "cd " + host->ultrainio_home;

  cmd = "cd " + host->ultrainio_home;
  if (!do_ssh(cmd, host->host_name)) {
    cerr << "Unable to switch to path " << host->ultrainio_home
...
void
launcher_def::launch (ultraind_def &instance, string &gts) {
  ...
  bfs::path pidf  = dd / "nodultrain.pid";
  host_def* host;
  try {
  ...
  string ultraindcmd = "programs/nodultrain/nodultrain ";
  if (skip_transaction_signatures) {
    ultraindcmd += "--skip-transaction-signatures ";
  }
  if (!ultraind_extra_args.empty()) {
    if (instance.name == "bios") {
       ...
       regex r("--plugin +ultrainio::mongo_db_plugin");
       string args = std::regex_replace (ultraind_extra_args,r,"");
       regex r2("--mongodb-uri +[^ ]+");
       args = std::regex_replace (args,r2,"");
       ultraindcmd += args + " ";
    }
    else {
       ultraindcmd += ultraind_extra_args + " ";
    }
  }

  if( add_enable_stale_production ) {
    ultraindcmd += "--enable-stale-production true ";
    add_enable_stale_production = false;
  }

  ultraindcmd += " --config-dir " + instance.config_dir_name + " --data-dir " + instance.data_dir_name;
  ultraindcmd += " --genesis-json " + instance.config_dir_name + "/genesis.json";
  if (gts.length()) {
    ultraindcmd += " --genesis-timestamp " + gts;
  }

  if (!host->is_local()) {
    string cmdl ("cd ");
    cmdl += host->ultrainio_home + "; nohup " + ultraindcmd + " > "
    ...
    string cmd = "cd " + host->ultrainio_home + "; kill -15 $(cat " + pidf.string() + ")";
    format_ssh (cmd, host->host_name, info.kill_cmd);
  }
  else {
    cerr << "spawning child, " << ultraindcmd << endl;

    bp::child c(ultraindcmd, bp::std_out > reout, bp::std_err > reerr );
    bfs::remove(reerr_sl);
    ...
    if(!c.running()) {
      cerr << "child not running after spawn " << ultraindcmd << endl;
      for (int i = 0; i > 0; i++) {
        if (c.running () ) break;
      }
    }
...
#if 0
void
launcher_def::kill_instance(ultraind_def, string sig_opt) {
}
#endif
...
vector<pair<host_def, ultraind_def>>
launcher_def::get_nodes(const string& node_number_list) {
   vector<pair<host_def, ultraind_def>> node_list;
...
launcher_def::bounce (const string& node_numbers) {
   ...
   for (auto node_pair: node_list) {
      ...
      const ultraind_def& node = node_pair.second;
      string node_num = node.name.substr( node.name.length() - 2 );
      string cmd = "cd " + host.ultrainio_home + "; "
                 + "export ULTRAINIO_HOME=" + host.ultrainio_home + string("; ")
                 + "export ULTRAINIO_TN_NODE=" + node_num + "; "
                 + "./scripts/ultrainio-tn_bounce.sh";
...
launcher_def::down (const string& node_numbers) {
   ...
   for (auto node_pair: node_list) {
      ...
      string node_num = node.name.substr( node.name.length() - 2 );
      string cmd = "cd " + host.ultrainio_home + "; "
                 + "export ULTRAINIO_HOME=" + host.ultrainio_home + "; "
                 + "export ULTRAINIO_TN_NODE=" + node_num + "; "
         + "export ULTRAINIO_TN_RESTART_CONFIG_DIR=" + node.config_dir_name + "; "
                 + "./scripts/ultrainio-tn_down.sh";
...
int main (int argc, char *argv[]) {
  ...
  cli.add_options()
    ...
    ("down", bpo::value<string>(&down_nodes),"comma-separated list of node numbers that will be taken down using the ultrainio-tn_down.sh script")
    ("bounce", bpo::value<string>(&bounce_nodes),"comma-separated list of node numbers that will be restarted using the ultrainio-tn_bounce.sh script")
    ("roll", bpo::value<string>(&roll_nodes),"comma-separated list of host names where the nodes should be rolled to a new version using the ultrainio-tn_roll.sh script")
  ...
  try {
    ...
    if (vmap.count("version") > 0) {
      cout << ultrainio::launcher::config::version_str << endl;
      return 0;
    }
...
FC_REFLECT( host_def,
            (genesis)(ssh_identity)(ssh_args)(ultrainio_home)
            (host_name)(public_name)
            (base_p2p_port)(base_http_port)(def_file_size)
            (instances) )

FC_REFLECT( ultraind_def,
...
FC_REFLECT( server_name_def, (ipaddr) (name) (has_bios) (ultrainio_home) (instances) )

FC_REFLECT( server_identities, (producer) (nonprod) (db) (default_ultrainio_home) (ssh) )
