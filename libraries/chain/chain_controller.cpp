...
#include <ultrainio/chain/contracts/contract_table_objects.hpp>
#include <ultrainio/chain/action_objects.hpp>
...
#include <ultrainio/chain/scope_sequence_object.hpp>
#include <ultrainio/chain/merkle.hpp>

#include <ultrainio/chain/exceptions.hpp>
#include <ultrainio/chain/wasm_interface.hpp>

#include <ultrainio/utilities/rand.hpp>
...
} ULTRAIN_CAPTURE_AND_RETHROW( transaction_exception ) }
...
{ try {
   ...
   if( should_check_authorization() ) {
      ...
      ULTRAIN_ASSERT( mtrx.delay >= enforced_delay,
...
void chain_controller::_finalize_block( const block_trace& trace, const producer_object& signing_producer ) { try {
   ...
   _resource_limits.set_block_parameters(
      {ULTRAIN_PERCENT(chain_config.max_block_cpu_usage, chain_config.target_block_cpu_usage_pct), chain_config.max_block_cpu_usage, config::block_cpu_usage_average_window_ms / config::block_interval_ms, 1000, {99, 100}, {1000, 999}},
      {ULTRAIN_PERCENT(chain_config.max_block_net_usage, chain_config.target_block_net_usage_pct), chain_config.max_block_net_usage, config::block_size_average_window_ms / config::block_interval_ms, 1000, {99, 100}, {1000, 999}}
   );
...
{ try {
   ...
   ULTRAIN_ASSERT( head_block.valid(), pop_empty_chain, "there are no blocks to pop" );
...
static void validate_shard_locks(const vector<shard_lock>& locks, const string& tag) {
   ...
   for (auto cur = locks.begin() + 1; cur != locks.end(); ++cur) {
      auto prev = cur - 1;
      ULTRAIN_ASSERT(*prev != *cur, block_lock_exception, "${tag} lock \"${a}::${s}\" is not unique", ("tag",tag)("a",cur->account)("s",cur->scope));
      ULTRAIN_ASSERT(*prev < *cur,  block_lock_exception, "${tag} locks are not sorted", ("tag",tag));
   }
}
...
{ try {
   ...
   for( uint32_t region_index = 0; region_index < next_block.regions.size(); ++region_index ) {
      ...
      ULTRAIN_ASSERT(!r.cycles_summary.empty(), tx_empty_region,"region[${r_index}] has no cycles", ("r_index",region_index));
      for (uint32_t cycle_index = 0; cycle_index < r.cycles_summary.size(); cycle_index++) {
         ...
         ULTRAIN_ASSERT(!cycle.empty(), tx_empty_cycle,"region[${r_index}] cycle[${c_index}] has no shards", ("r_index",region_index)("c_index",cycle_index));
         for (uint32_t shard_index = 0; shard_index < cycle.size(); shard_index++) {
            const auto& shard = cycle.at(shard_index);
            ULTRAIN_ASSERT(!shard.empty(), tx_empty_shard,"region[${r_index}] cycle[${c_index}] shard[${s_index}] is empty",
            ...
            for (const auto& s: shard.read_locks) {
               ULTRAIN_ASSERT(write_locks.count(s) == 0, block_concurrency_exception,
            ...
            for (const auto& s: shard.write_locks) {
               ...
               ULTRAIN_ASSERT(read_locks.count(s) == 0, block_concurrency_exception,
            ...
            for (const auto& receipt : shard.transactions) {
                ...
                auto make_metadata = [&]() -> transaction_metadata* {
                  ...
                  if( itr != trx_index.end() ) {
                     ...
                     if( should_check_authorization() ) {
                        ...
                        ULTRAIN_ASSERT( trx_meta.delay >= enforced_delay,
                                    transaction_exception,
                                    "authorization imposes a delay (${enforced_delay} sec) greater than the delay specified in transaction header (${specified_delay} sec)",
               ...
               ULTRAIN_ASSERT( receipt.status == s_trace.transaction_traces.back().status, tx_receipt_inconsistent_status,
               ...
               ULTRAIN_ASSERT( receipt.kcpu_usage == s_trace.transaction_traces.back().kcpu_usage, tx_receipt_inconsistent_cpu,
               ...
               ULTRAIN_ASSERT( receipt.net_usage_words == s_trace.transaction_traces.back().net_usage_words, tx_receipt_inconsistent_net,
            ...
            ULTRAIN_ASSERT( boost::equal( used_read_locks, shard.read_locks ),
               block_lock_exception, "Read locks for executing shard: ${s} do not match those listed in the block", ("s", shard_index));
            ULTRAIN_ASSERT( boost::equal( used_write_locks, shard.write_locks ),
   ...
   for (const auto& act : trx.actions ) {
      for (const auto& declared_auth : act.authorization) {
         if (!checker.satisfied(declared_auth)) {
            ULTRAIN_ASSERT(checker.satisfied(declared_auth), tx_missing_sigs,
         ...
         if( act.account == config::system_account_name ) {
            ...
            if( act.name == contracts::linkauth::get_name() ) {
               ...
               if( declared_auth.actor == link.account ) {
                  ...
                  if( linked_permission_name.valid() && *linked_permission_name != config::ultrainio_any_name ) {
               ...
               if( declared_auth.actor == unlink.account ) {
                  ...
                  if( unlinked_permission_name.valid() && *unlinked_permission_name != config::ultrainio_any_name ) {
...
void chain_controller::validate_uniqueness( const transaction& trx )const {
   ...
   ULTRAIN_ASSERT(transaction == nullptr, tx_duplicate, "Transaction is not unique");
}
...
void chain_controller::record_transaction(const transaction& trx)
{
   try {
       ...
       ULTRAIN_ASSERT( false, transaction_exception,
...
static uint32_t calculate_transaction_cpu_usage( const transaction_trace& trace, const transaction_metadata& meta, const chain_config& chain_configuration ) {
   ...
   ULTRAIN_ASSERT( cpu_usage_limit == 0 || actual_cpu_usage <= cpu_usage_limit, tx_resource_exhausted,
...
static uint32_t calculate_transaction_net_usage( const transaction_trace& trace, const transaction_metadata& meta, const chain_config& chain_configuration ) {
   ...
   ULTRAIN_ASSERT( net_usage_limit == 0 || actual_net_usage <= net_usage_limit, tx_resource_exhausted,
...
void chain_controller::update_resource_usage( transaction_trace& trace, const transaction_metadata& meta ) {
   ...
   ULTRAIN_ASSERT(trace.cpu_usage <= chain_configuration.max_transaction_cpu_usage,
   ...
   ULTRAIN_ASSERT(trace.net_usage <= chain_configuration.max_transaction_net_usage,
...
{ try {
   ...
   ULTRAIN_ASSERT( now < time_point(trx.expiration),
...
{ try {
   ...
   ULTRAIN_ASSERT( time_point(trx.expiration) <= reference_time + fc::seconds(chain_configuration.max_transaction_lifetime),
...
{ try {
   ULTRAIN_ASSERT( !trx.actions.empty(), tx_no_action, "transaction must have at least one action" );
   ...
   ULTRAIN_ASSERT( has_auth, tx_no_auths, "transaction must have at least one authorization" );

   // Check that there are no authorizations in any of the context-free actions
   for (const auto &act : trx.context_free_actions) {
      ULTRAIN_ASSERT( act.authorization.empty(), cfa_irrelevant_auth, "context-free actions cannot require authorization" );
   }

   ULTRAIN_ASSERT( trx.max_kcpu_usage.value < UINT32_MAX / 1024UL, transaction_exception, "declared max_kcpu_usage overflows when expanded to max cpu usage" );
   ULTRAIN_ASSERT( trx.max_net_usage_words.value < UINT32_MAX / 8UL, transaction_exception, "declared max_net_usage_words overflows when expanded to max net usage" );
...
{ try {
   ...
   ULTRAIN_ASSERT( net_usage_limit == 0 || min_net_usage <= net_usage_limit,
               transaction_exception,
...
void chain_controller::require_scope( const scope_name& scope )const {
   switch( uint64_t(scope) ) {
      case config::ultrainio_all_scope:
      case config::ultrainio_auth_scope:
...
const producer_object& chain_controller::validate_block_header(uint32_t skip, const signed_block& next_block)const { try {
   ULTRAIN_ASSERT(head_block_id() == next_block.previous, block_validate_exception, "",
              ("head_block_id",head_block_id())("next.prev",next_block.previous));
   ULTRAIN_ASSERT(head_block_time() < (fc::time_point)next_block.timestamp, block_validate_exception, "",
   ...
   if( !is_start_of_round( next_block.block_num() ) )  {
      ULTRAIN_ASSERT(!next_block.new_producers, block_validate_exception,
   ...
   if(!(skip&skip_producer_signature))
      ULTRAIN_ASSERT(next_block.validate_signee(producer.signing_key), block_validate_exception,
   ...
   if(!(skip&skip_producer_schedule_check)) {
      ULTRAIN_ASSERT(next_block.producer == producer.owner, block_validate_exception,
   ...
   ULTRAIN_ASSERT( next_block.schedule_version == expected_schedule_version , block_validate_exception,"wrong producer schedule version specified ${x} expectd ${y}",
...
void chain_controller::_update_producers_authority() {
   const auto& gpo = get_global_properties();
   uint32_t authority_threshold = ULTRAIN_PERCENT_CEIL(gpo.active_producers.producers.size(), config::producers_authority_threshold_pct);
...
ProducerRound chain_controller::calculate_next_round(const signed_block& next_block) {
   ...
   ULTRAIN_ASSERT(boost::range::equal(next_block.producer_changes, changes), block_validate_exception,
...
void chain_controller::update_last_irreversible_block()
{
   ...
   size_t offset = ULTRAIN_PERCENT(producer_objs.size(), config::percent_100- config::irreversible_threshold_percent);
