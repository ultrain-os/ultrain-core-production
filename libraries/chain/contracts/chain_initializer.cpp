...
#include <ultrainio/chain/contracts/ultrain_contract.hpp>
...
#define SET_APP_HANDLER( contract, scope, action, nspace ) \
   ...
   SET_APP_HANDLER( ultrainio, ultrainio, newaccount, ultrainio );
   SET_APP_HANDLER( ultrainio, ultrainio, setcode, ultrainio );
   SET_APP_HANDLER( ultrainio, ultrainio, setabi, ultrainio );
   SET_APP_HANDLER( ultrainio, ultrainio, updateauth, ultrainio );
   SET_APP_HANDLER( ultrainio, ultrainio, deleteauth, ultrainio );
   SET_APP_HANDLER( ultrainio, ultrainio, linkauth, ultrainio );
   SET_APP_HANDLER( ultrainio, ultrainio, unlinkauth, ultrainio );
   SET_APP_HANDLER( ultrainio, ultrainio, onerror, ultrainio );
   SET_APP_HANDLER( ultrainio, ultrainio, postrecovery, ultrainio );
   SET_APP_HANDLER( ultrainio, ultrainio, passrecovery, ultrainio );
   SET_APP_HANDLER( ultrainio, ultrainio, vetorecovery, ultrainio );
   SET_APP_HANDLER( ultrainio, ultrainio, canceldelay, ultrainio );
}


abi_def chain_initializer::ultrain_contract_abi(const abi_def& ultrainio_system_abi)
{
   abi_def ultrain_abi(ultrainio_system_abi);
   ultrain_abi.types.push_back( type_def{"account_name","name"} );
   ultrain_abi.types.push_back( type_def{"table_name","name"} );
   ultrain_abi.types.push_back( type_def{"share_type","int64"} );
   ultrain_abi.types.push_back( type_def{"onerror","bytes"} );
   ultrain_abi.types.push_back( type_def{"context_free_type","bytes"} );
   ultrain_abi.types.push_back( type_def{"weight_type","uint16"} );
   ultrain_abi.types.push_back( type_def{"fields","field[]"} );
   ultrain_abi.types.push_back( type_def{"time_point_sec","time"} );
   ...
   ultrain_abi.actions.push_back( action_def{name("postrecovery"), "postrecovery",""} );
   ultrain_abi.actions.push_back( action_def{name("passrecovery"), "passrecovery",""} );
   ultrain_abi.actions.push_back( action_def{name("vetorecovery"), "vetorecovery",""} );
   ...
   ultrain_abi.tables.emplace_back( table_def {
      "recovery", "i64", {
         "account",
      }, {
         "name"
