...
void apply_ultrainio_newaccount(apply_context& context) {
   ...
   ULTRAIN_ASSERT( validate(create.recovery), action_validate_exception, "Invalid recovery authority");
   ...
   if( !creator.privileged ) {
      ULTRAIN_ASSERT( name_str.find( "ultrainio." ) != 0, action_validate_exception,
                  "only privileged accounts can have names that start with 'ultrainio.'" );
   }
...
void apply_ultrainio_setabi(apply_context& context) {
   ...
   if ( act.account == ultrainio::chain::config::system_account_name ) {
      act.abi = chain_initializer::ultrain_contract_abi(act.abi);
   }
...
void apply_ultrainio_updateauth(apply_context& context) {
   ...
   ULTRAIN_ASSERT( update.permission.to_string().find( "ultrainio." ) != 0, action_validate_exception,
               "Permission names that start with 'ultrainio.' are reserved" );
   ...
   ULTRAIN_ASSERT(validate(update.data), action_validate_exception,
   ...
   auto permission_is_valid_for_update = [&](){
      ...
      ULTRAIN_ASSERT(current != nullptr, permission_query_exception,
...
void apply_ultrainio_onerror(apply_context& context) {
...
static const abi_serializer& get_abi_serializer() {
   ...
   if (!_abi_serializer) {
      _abi_serializer.emplace(chain_initializer::ultrain_contract_abi(abi_def()));
   }
...
void apply_ultrainio_postrecovery(apply_context& context) {
   context.require_write_lock( config::ultrainio_auth_scope );
...
void apply_ultrainio_passrecovery(apply_context& context) {
...
void apply_ultrainio_vetorecovery(apply_context& context) {
   context.require_write_lock( config::ultrainio_auth_scope );
