...
namespace ultrainio { namespace wallet {
...
public:
   ...
   private_key_type get_private_key(const public_key_type& id)const
   {
      ...
      ULTRAIN_ASSERT( has_key, chain::key_nonexistent_exception, "Key doesn't exist!" );
      return *has_key;
   }
   ...
   bool import_key(string wif_key)
   {
      ...
      ultrainio::chain::public_key_type wif_pub_key = priv.get_public_key();
      ...
      ULTRAIN_THROW( chain::key_exist_exception, "Key already in wallet" );
   }
   ...
   bool remove_key(string key)
   {
      ...
      ULTRAIN_THROW( chain::key_nonexistent_exception, "Key not in wallet" );
   }
   ...
   void save_wallet_file(string wallet_filename = "")
   {
      ...
      try
      {
         ...
         if (!outfile) {
            ...
            ULTRAIN_THROW(wallet_exception, "Unable to open file: ${fn}", ("fn", wallet_filename));
         }
...
{
   ULTRAIN_ASSERT(!is_locked(), wallet_locked_exception, "Unable to import key on a locked wallet");
...
{
   ULTRAIN_ASSERT(!is_locked(), wallet_locked_exception, "Unable to remove key from a locked wallet");
...
{
   ULTRAIN_ASSERT(!is_locked(), wallet_locked_exception, "Unable to create key on a locked wallet");
...
{ try {
   ULTRAIN_ASSERT( !is_locked(), wallet_locked_exception, "Unable to lock a locked wallet" );
   encrypt_keys();
...
{
   ULTRAIN_ASSERT(!is_locked(), wallet_locked_exception, "Unable to list public keys of a locked wallet");
   return my->_keys;
}

flat_set<public_key_type> soft_wallet::list_public_keys() {
   ULTRAIN_ASSERT(!is_locked(), wallet_locked_exception, "Unable to list private keys of a locked wallet");
