...
#include <ultrainio/state_history_plugin/state_history_log.hpp>
...
bool include_delta(const ultrainio::chain::table_id_object& old, const ultrainio::chain::table_id_object& curr) {
   return old.payer != curr.payer;
}

bool include_delta(const ultrainio::chain::resource_limits::resource_limits_object& old,
                   const ultrainio::chain::resource_limits::resource_limits_object& curr) {
   return                                   //
...
bool include_delta(const ultrainio::chain::resource_limits::resource_limits_state_object& old,
                   const ultrainio::chain::resource_limits::resource_limits_state_object& curr) {
   return                                                                                       //
...
struct state_history_plugin_impl : std::enable_shared_from_this<state_history_plugin_impl> {
   ...
   void listen() {
      ...
      auto check_ec = [&](const char* what) {
         if (!ec)
            return;
         elog("${w}: ${m}", ("w", what)("m", ec.message()));
         ULTRAIN_ASSERT(false, plugin_exception, "unable to open listen socket");
      };
   ...
   static bool is_onblock(const transaction_trace_ptr& p) {
      ...
      if (act.account != ultrainio::chain::config::system_account_name || act.name != NEX(onblock) ||
          act.authorization.size() != 1)
         return false;
      auto& auth = act.authorization[0];
      return auth.actor == ultrainio::chain::config::system_account_name &&
             auth.permission == ultrainio::chain::config::active_name;
   }
   ...
   void on_applied_transaction(const transaction_trace_ptr& p) {
      if (p->receipt && trace_log) {
         if (is_onblock(p))
            ...
            cached_traces[p->failed_dtrx_trace->id] = augmented_transaction_trace{p};
         else
            cached_traces[p->id] = augmented_transaction_trace{p};
      }
   }
   ...
   void store_traces(const block_state_ptr& block_state) {
      ...
      for (auto& r : block_state->block->transactions) {
         ...
         else if(r.trx.contains<packed_generated_transaction>())
            id = r.trx.get<packed_generated_transaction>().id();
         else
            id = r.trx.get<packed_transaction>().id();
         auto it = cached_traces.find(id);
         ULTRAIN_ASSERT(it != cached_traces.end() && it->second.trace->receipt, plugin_exception,
      ...
      ULTRAIN_ASSERT(traces_bin.size() == (uint32_t)traces_bin.size(), plugin_exception, "traces is too big");
   ...
   void store_chain_state(const block_state_ptr& block_state) {
      ...
      auto process_table = [&](auto* name, auto& index, auto& pack_row) {
          if (fresh && (strcmp(name,"account")==0)) {
      ...
      /*
      // comment out all these tables as we only need to store talbes in contracts
      process_table("account", db.get_index<account_index>(), pack_row);
      ...
      */
      ...
      ULTRAIN_ASSERT(deltas_bin.size() == (uint32_t)deltas_bin.size(), plugin_exception, "deltas is too big");
      state_history_log_header header{.magic        = ship_magic(ship_current_version),
                                      .block_id     = block_state->block->id(),
