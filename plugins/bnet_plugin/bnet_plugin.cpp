...
#include <ultrainio/bnet_plugin/bnet_plugin.hpp>
...
using ultrainio::public_key_type;
using ultrainio::chain_id_type;
using ultrainio::block_id_type;
using ultrainio::block_timestamp_type;
using std::string;
using ultrainio::sha256;
using ultrainio::signed_block_ptr;
using ultrainio::packed_transaction_ptr;
using std::vector;
...
namespace ultrainio {
       ...
       //    shared_ptr< vector<char> >  block_msg; ///< packed bnet_message for this block
        ...
        void on( const signed_block_ptr& b ) {
           ...
           if (!b) {
              ...
              ULTRAIN_THROW(block_validate_exception, "bad block" );
           }
        ...
        void on( const packed_transaction_ptr& p ) {
           ...
           if (!p) {
              ...
              ULTRAIN_THROW(transaction_exception, "bad transaction");
           }
  ...
  class listener : public std::enable_shared_from_this<listener> {
     ...
     public:
        ...
        void run() {
           ULTRAIN_ASSERT( _acceptor.is_open(), plugin_exception, "unable top open listen socket" );
           do_accept();
        }
   ...
   void bnet_plugin::plugin_shutdown() {
      ...
      my->for_each_session([](auto ses){
         ULTRAIN_ASSERT( false, plugin_exception, "session ${ses} still active", ("ses", ses->_session_num) );
      });
