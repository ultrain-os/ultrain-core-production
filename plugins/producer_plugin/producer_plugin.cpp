...
class producer_plugin_impl : public std::enable_shared_from_this<producer_plugin_impl> {
   public:
      ...
      uint32_t _production_skip_flags              = 0; //ultrainio::chain::skip_nothing;
...
void new_chain_banner(const ultrainio::chain::controller& db)
{
   std::cerr << "\n"
      "*******************************\n"
      "*                             *\n"
...
static producer_plugin_impl::signature_provider_type
make_kultraind_signature_provider(const std::shared_ptr<producer_plugin_impl>& impl, const string& url_str, const public_key_type pubkey) {
...
{ try {
   ...
   if (!my->_producers.empty()) {
      ...
      if (my->_production_enabled) {
         ...
         //_production_skip_flags |= ultrainio::chain::skip_undo_history_check;
      }
   }
