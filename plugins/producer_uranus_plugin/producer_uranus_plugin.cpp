...
public:
   ...
   uint32_t _production_skip_flags              = ultrainio::chain::skip_nothing;
...
void new_chain_banner(const ultrainio::chain::chain_controller& db)
{
   std::cerr << "\n"
      "*******************************\n"
      "*                             *\n"
      "*   ------ NEW CHAIN ------   *\n"
      "*   -  Welcome to ULTRAINIO!  -   *\n"
      "*   -----------------------   *\n"
      "*                             *\n"
      "*******************************\n"
      "\n";
...
chain::public_key_type producer_uranus_plugin::first_producer_public_key() const
{
...
{ try {
   ...
   if (!my->_producers.empty())
   {
      ...
      if(my->_production_enabled)
      {
         ...
         my->_production_skip_flags |= ultrainio::chain::skip_undo_history_check;
      }
...
void producer_uranus_plugin_impl::schedule_production_loop() {
   ...
   _timer.expires_from_now( boost::posix_time::microseconds(time_to_next_block_time) );
   //_timer.async_wait(boost::bind(&producer_uranus_plugin_impl::block_production_loop, this));
   _timer.async_wait( [&](const boost::system::error_code&){ block_production_loop(); } );
}

block_production_condition::block_production_condition_enum producer_uranus_plugin_impl::block_production_loop() {
...
block_production_condition::block_production_condition_enum producer_uranus_plugin_impl::maybe_produce_block(fc::mutable_variant_object& capture) {
   chain::chain_controller& chain = app().get_plugin<chain_plugin>().chain();
   fc::time_point now = fc::time_point::now();
   ...
   ultrainio::chain::public_key_type scheduled_key = chain.get_producer(scheduled_producer).signing_key;
